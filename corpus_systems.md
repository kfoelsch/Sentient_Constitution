...
*Systems companion — binding systems-and-data implementation annex. Prefer the filename in cross-references; the bare phrase “Constitutional Systems” is not used in corpus body text (see [doc_architecture.md](doc_architecture.md) — Plain-Language Vocabulary Guardrails, Ambiguous implementation labels).*
...
*See [doc_architecture.md](doc_architecture.md) section **17**.* Definition boundaries between this annex and the core constitution are in `doc_architecture.md` section 4 (Project-wide definitions protocol).

System and data obligations in this annex align with [corpus_joint_structure.md — Cross-domain implementation layer](corpus_joint_structure.md#cross-domain-implementation-layer) (Implementation Groups One through Four).

Mapping summary:
- Technical requirements map primarily to **PRIM1**, **PRIM2**, **PRIM4–PRIM12**, and **PRIM14–PRIM15**
- Governance, proportionality of authority, justification, challenge, and collective-choice processes map to **PROT1–PROT6** together with Chapter Ten decision-resolution requirements
...
- **Chapter Nine structure:** The Sentient Constitution **Chapter Nine** opening states a **default constraint stack** and **interpretive hubs**:
  ...
  - This annex implements those themes operationally and must not narrow them. **Article XIV-A** info-sphere plurality, **Article XIV-B** contestability, and **Article XV-A** auditability floors are supplemented by Article XV's remaining transparency and verification provisions and annex protocols for **high-impact** summarization, ranking, and mediation systems. **Article VIII-C** publication limits are elaborated in **Chapter Five**, section 3 — Dependent clusters ([Truth and Epistemic Integrity](core_05-05_definitions_c_dependent_clusters.md#truth-and-epistemic-integrity-cluster); **Publication and High-Impact Communication**).
...
Rights-level statements for environment separation, reversibility, sandboxed innovation, transition to higher-impact operation, and misclassification consequences appear in Sentient Constitution Chapter Nine. **See** Articles XVI and XVII and cross-references in those Articles. This Protocol supplies engineering and deployment profiles that implement, rather than restate, those articles. Normative implementation label detail is in **corpus_joint_structure.md** (PRIM/PROT codes referenced throughout this annex). This Protocol is an operational profile, not a second home for those implementation labels. This constitution defines required capabilities and outcomes. Specific technical implementations may evolve, provided they remain auditable and aligned with constitutional principles. System categories in this Protocol are functional and may overlap. Where multiple classifications apply, the stricter safeguards govern.
...
**Consent:** genuine opt-in to experimental substrate-related systems. No coerced participation (**Sentient Constitution Chapter Nine**, **Articles V–XXV**, **Chapter One** constraints).
...
**G. Governance continuity, crisis communications, and exercises (high-impact systems).** This subsection states governance-side business continuity and recovery expectations. It applies to **Class A** and **Class B** systems, and to materially affecting **Critical System Stewards** (Chapter S3 — Critical System Stewardship). It complements technical resilience, environment separation, and testing elsewhere in this Protocol, and it complements **corpus_joint_structure.md**, Implementation Group One (*Failure Integrity* Meta-integrity obligation), incorporated via **Sentient Constitution Chapter Fourteen**. It does **not** create constitutional rights.

Crisis and emergency measures remain governed by **Sentient Constitution Chapter Five** (*Emergency and Contingency*; *Force Majeure*). Procedural emergency controls, conflict resolution, proportionality, and review of restrictions and emergency measures remain governed by **Chapter Nine, Article XXIII** (*Conflict Resolution, Escalation, and Emergency Proportionality*) as referenced across this corpus. They also remain governed by Chapter Ten decision-resolution requirements and **corpus_joint_structure.md** **PROT2** (*Intervention and Override Rights*).
...
Define pre-authorized boundaries for expedited action where **Article XXIII** and Chapter Ten decision-resolution requirements permit.
...
That review must be consistent with **Chapter Nine, Article XXIII-A** and **corpus_joint_structure.md PROT6**.
...
Continuation past default expiry requires documented reauthorization. That record must show ongoing necessity and proportionality (**PROT1**; **Chapter One** and **Chapter Five** where definitional or scaling detail applies). It must also show why less-restrictive feasible alternatives are insufficient.
...
This protocol states annex-level expectations for understandable systems and manageable complexity. It implements Sentient Constitution Chapter Nine, Article XX-A and Article XX-B. It does **not** duplicate `corpus_joint_structure.md` or restate Article XX in full. Normative detail for cognitive accessibility, transparent presentation, dependency and modularity, failure behavior, and adversarial robustness appears in `corpus_joint_structure.md` **CJS-3.15** (comprehensibility and cognitive accessibility), **CJS-3.16** (salience integrity and attention allocation), **CJS-3.17** (disclosure sufficiency and observability), **CJS-3.18** (dependency integrity and disclosure), **CJS-3.19** (graceful degradation and failure-mode integrity), and **CJS-3.27** (adversarial robustness and abuse resistance).

Where Protocol B and an implementation label conflict, the stricter requirement governs. Where this protocol is silent on interpretive, definitional, verification, or traceability standards for constitutional terms, **Sentient Constitution Chapters Two through Four** governs.

**Sentient Constitution Chapter Five** governs as well.

**Classification-scaled application:** Expectations for comprehensibility, complexity audits, modularity, and oversight capacity scale with **system class**. **They** also scale with **Critical System Steward** tier (CSS-A / CSS-B / CSS-C) under **Chapter S3**.

The **Comprehensibility and Complexity Stewardship** line in each **Implementation label Application Profile** under **Chapter S2** is authoritative for classified systems. **Those** profiles cover Classes A, B, C, L, and P. **It** governs how Protocol B and the corresponding cross-companion operational definitions in `corpus_joint_structure.md` **CJS-3.15** through **CJS-3.19** and **CJS-3.27** intensify or relax together with transparency and auditability.

**Chapter S3** adds organization-scaled rows. Where both apply, **stricter** governs.

Growing complexity must **not** function as a barrier to audit, participation, or accountability beyond what **Article XX** allows.

Proportionality scaled under **PROT1** (and applicable **Sentient Constitution** **Chapter One** and **Chapter Five**) governs how far **Article XX** permits such barriers.

Opaque or unmanageable structures threaten the survival, autonomy, and wellbeing this constitution protects.

**Comprehensibility:** In furtherance of **Article XX-A**, systems affecting survival, environment, information integrity, and sentient autonomy must remain understandable. **That** understandability applies to those responsible for operation and oversight.

**Understandability** must be at levels proportionate to classification and risk (see `corpus_joint_structure.md` **CJS-3.15** where cross-companion application is material).

**Complexity audits:** Critical systems must periodically undergo independent complexity audits. Findings should be publicly documented whenever possible.

Audits evaluate **system transparency and observability**, **dependency chains and hidden coupling**, **failure modes and cascading risks**, and **capacity for human oversight and intervention**.

**Modular system architecture:** Design as modular components with clear responsibilities and interfaces.

Innovations in one component must **not** produce unintended consequences across other systems.

**Redundancy and resilience:** Critical infrastructure must incorporate redundancy and fail-safe mechanisms to prevent single points of failure.

Where possible, maintain multiple independent implementations of essential systems.

**Incident learning:** Treat failures, anomalies, and near-misses as opportunities for collective learning.

Post-incident reviews must be transparent and focus on systemic weaknesses rather than blame.

**Periodic simplification:** Governance institutions periodically review systems for unnecessary complexity.

Where possible, simplify or remove redundant processes, outdated rules, or excessive dependencies to preserve clarity, operability, and resilience.

## Protocol C: Justice Safeguards, Restitution, and Rehabilitation Implementation

Constitutional tracing: This protocol implements Sentient Constitution Chapter Nine, **Article XXIII** (*Conflict Resolution, Escalation, and Emergency Proportionality*). **It** also implements **Sentient Constitution Chapter Six** (compliance, violation, and standing model) and `corpus_joint_structure.md` **CJS-3.33** (cross-companion procedural integrity and adjudication terms), read with **corpus_joint_structure.md PROT6**. **It** operationalizes the justice objective that interventions remain safety-protective, restitution-oriented, rehabilitation-capable where feasible, and accountability-grounded.
...
Apply Sentient Constitution **Chapter Nine** (*Disputes Involving External Legal Orders*) where applicable.
...
or layered delegation intended to dilute accountability must trigger aggravated enforcement review. It must also trigger anti-evasion intervention under **Sentient Constitution Chapters Two through Four** and **PROT6**.
...
### I. Purpose and scope
**Sentient Constitution Chapter Nine** (Articles **I**–**XXV**; presentation **Parts A–D**) states foundational rights that depend upon strong, reproducible procedures and governance. That includes info-sphere, audit, and comprehensibility hooks where they apply to data handling (e.g., **Articles XIV**, **XV**, and **XX**).
...
Handling must align with Sentient Constitution Chapter Nine and scale with system class under Chapter S2.
...
All data access, processing, and transformation must be **attributable** to identifiable systems, agents, or sentients (**Chapter Nine, Article VII** — self-ownership and attributable representation where applicable) and **recorded** in a manner sufficient for audit and reconstruction (`corpus_joint_structure.md` **CJS-3.22**). Systems must ensure **clear responsibility** for actions taken on data and **traceability** of decisions and outcomes.
...
PRIM11, PROT4, PROT5, **Sentient Constitution Chapter Nine, Article VII-B** (*Internal-State Boundary and Type-N Protection*), **corpus_joint_structure.md**, **Implementation Group One** (meta-integrity obligations), incorporated via **Sentient Constitution Chapter Fourteen**.
...
**Safety, security, and abuse:** **Necessity** and **Proportionality** justify **narrow** **fraud**, **security**, and **abuse** controls. The target is **pretextual** or **concentration-driven** denial, **not** **forced service** for **materially harmful** use. Read with **Article V-G** (*Accessibility*), **Article V-H** (*Expression, Assembly, and Press*), **Article X-A** (*Non-Imposition and Consent in Association*), **Article XIII-A** (*Security, Intelligence, and Covert-Power Limits*), **Article XVIII-D** (*Movement, Migration, and Refuge*), **Article XIX** (*Interoperability, Portability, and Exit Integrity*), **Chapter One §5.1** (*Productive Capacity* and non-concentration), `corpus_joint_structure.md` interoperability and **exit** implementation labels, and **`corpus_institutions.md` CI-21** (commons and mutual-aid coordination).
...
the **stricter** governs (**Protocol A**/**Protocol B** in this annex; **doc_architecture.md** precedence).
...
**I. Self-healing and recovery-path integration (Article XII-F implementation profile).** This subsection ties this Protocol's adaptive-allocation and root-cause architecture to **Sentient Constitution Chapter Nine, Article XII-F** (*Resilience and Self-Healing Baseline*), **Chapter One §4.1** (*Resilience and Self-Healing Design*), and **Chapter Five** [*Self-Healing*](core_05-05_definitions_a_independent.md#self-healing-constitutional). It does **not** restate or narrow those homes. Where this subsection is silent, Chapter One §4.1, Article XII-F, and the Chapter Five definition govern.

**Adaptive recovery as constitutional self-healing:** Adaptive allocation, cause-aligned mitigation, and ecosystem risk response under sections **B**–**G** of this Protocol are the ecosystem-level operationalization of self-healing. They must be evaluated against **Article XII-F**'s Detection, Containment, Safe-Failure Preference, Non-Masking, Rights-Floor Continuity, Autonomy Scaling, and Root-Cause Closure bullets in addition to the sustainability-oriented criteria in this Protocol.

**Non-masking in root-cause analysis:** Root cause analysis under section **C.1** must satisfy **Article XII-F**'s Non-Masking bullet as well as the documentation, auditability, and contestability standards of **Article XV-A** and **Article XXI-A**. Adaptive reallocation must not be permitted to suppress, overwrite, or obscure the fault signals that triggered it, nor to converge into a suppression loop in which recurring conditions are reliably silenced before they can be independently observed. Recurrence across cycles must be treated as a single open root-cause obligation under section **C.1** and **Article XXI**.

**Rights-floor continuity under ecosystem stress:** Where adaptive response narrows capacity that implements Chapter Nine guarantees (for example, contestability capacity, audit fidelity, participation access), narrowing must not be silent. Silent narrowing under the banner of ecosystem recovery is non-compliant under **Article XII-F**'s Rights-Floor Continuity bullet, **Article XII-C** (prohibition of false trust), and **Chapter One §6.1.4** burden-reduction limits. Escalation to **Article XXV** transition-governance applies where degraded modes persist beyond pre-declared thresholds.

**Containment and anti-concentration:** Adaptive allocation must bound blast radius consistent with **Article XII-F**'s Containment bullet. Recovery actions must not propagate failure through [Cascading Failure](core_05-05_definitions_a_independent.md#cascading-failure) pathways; must not alter persistent state, credentials, obligations, or configurations attributed to sentients, operators, or other systems **outside** the declared fault-and-recovery scope except through changes that satisfy **Article XV-A** [Auditability](core_05-05_definitions_c_dependent_clusters.md#auditability) for observability and attribution and that, where others are materially affected, include proportionate notice, authorization, or contestable handoff consistent with **Chapter Nine**; and must not expand authority beyond the pre-fault envelope. Anti-concentration obligations under section **F** apply to recovery-time authority as well as steady-state authority: recovery must not become a vector for concentration of resources, influence, or dependencies.

**Governance non-substitution:** Section **H**'s governance-integration rule is read with **Chapter One §4.1**'s non-substitution-for-governance clause. No adaptive or self-healing mechanism may substitute for governance, reduce [Auditability](core_05-05_definitions_c_dependent_clusters.md#auditability), [Transparency](core_05-05_definitions_c_dependent_clusters.md#transparency), contestability, or stewardship obligations under [Incentive Alignment](core_05-05_definitions_a_independent.md#incentive-alignment). High-autonomy adaptive behavior remains subject to **Article XII-E** and to section **H** of this Protocol jointly.
...
**8. Self-healing under decentralized continuity (Article XII-F implementation profile).** This section specifies how self-healing behavior must operate across the four continuity modes defined in section **1** — **Normal**, **Degraded-Partitioned**, **Offline-Sovereign**, and **Rejoin-Reconciliation**. It implements **Sentient Constitution Chapter Nine, Article XII-F** (*Resilience and Self-Healing Baseline*), **Chapter One §4.1** (*Resilience and Self-Healing Design*), and **Chapter Five** [*Self-Healing*](core_05-05_definitions_a_independent.md#self-healing-constitutional). It does **not** restate or narrow those homes. Where this section is silent, Chapter One §4.1, Article XII-F, and the Chapter Five definition govern.

**Detection and observability across modes:** Each continuity mode must provide timely, observable detection of fault, degradation, or constitutional-constraint violation sufficient to satisfy **Article XII-F**'s Detection bullet and **Article XV-A** for the recovery path itself. Offline and partitioned modes must maintain tamper-evident local recovery-event chains consistent with section **4** (offline audit integrity) and must reconcile recovery events on rejoin rather than treating mode-internal recovery as closed.

**Containment across partitions:** Recovery actions must bound blast radius consistent with **Article XII-F**'s Containment bullet. Recovery across partitions must not alter persistent state, credentials, obligations, or configurations attributed to sentients, operators, or other systems **in other partitions** that fall **outside** the declared fault-and-recovery scope except through changes that satisfy **Article XV-A** [Auditability](core_05-05_definitions_c_dependent_clusters.md#auditability) for observability and attribution and that, where parties in those partitions are materially affected, include proportionate notice, authorization, or contestable handoff consistent with **Chapter Nine**; must not expand authority beyond the pre-fault envelope in any partition; and must not propagate failure through [Cascading Failure](core_05-05_definitions_a_independent.md#cascading-failure) pathways exposed by partition topology.

**Safe-failure preference under partition uncertainty:** Partitioned and offline-sovereign modes carry elevated uncertainty about global state. Safe failure, quarantine, or controlled handoff must be preferred over speculative auto-repair, consistent with **Article XII-F**'s Safe-Failure Preference bullet and section **3**'s partition-safe decision constraints. Recovery actions taken under partition that prove invalid on rejoin must be subject to section **7**'s correction, reversal, or remediation pathway.

**Non-masking during partition and rejoin:** Recovery actions, recovery attempts, and suppressed recovery attempts across all four modes are themselves auditable events under **Article XV-A** and **Article XII-F**'s Non-Masking bullet. Mode transitions, provisional trust states under section **5.D**, and emergency-authority invocations must not be permitted to suppress, overwrite, or delay evidence needed for root cause analysis under **Article XXI-A**. Reconciliation on rejoin must treat masked or under-logged recovery as a post-rejoin validation failure under section **7**.

**Rights-floor continuity under partition:** Degraded-Partitioned and Offline-Sovereign modes must preserve the **Chapter Nine** rights floor, or must escalate rather than silently narrow it. Section **1**'s rights-floor invariants and **Article XII-F**'s Rights-Floor Continuity bullet control; partition conditions are not by themselves sufficient justification for silent narrowing. Where rights-floor capacity is genuinely constrained by partition topology, narrowing must be explicit, time-bound, and restoration-triggered, and must be treated as **Article XXV** transition-governance territory at rejoin.

**Autonomy scaling under decentralized recovery:** Autonomous recovery by high-autonomy systems operating across continuity modes is subject to **Article XII-E** and **Article XII-F**'s Autonomy Scaling bullet. Recovery authority during partition must not be used to bypass [Contestability](core_05-05_definitions_b_semi_independent.md#contestability), challenge under **Article XII-B**, or independent verification under **Article XV-A** and Article XV's verification-access provisions. Internalization of contestability intake, audit emission, or external-review pathways during partition — for example, re-pointing contestability to a partition-local queue controlled by the same system that is exercising recovery authority — is prohibited; such channels must remain materially external or independently verifiable within the partition and must reconcile on rejoin.

**Root-cause closure across rejoin:** Self-healing that succeeds operationally in a partitioned or offline mode but leaves a known defective condition in place is a conditional state. It must carry an open root-cause obligation under **Article XXI-A** into Rejoin-Reconciliation mode (section **7**) and a disclosed expected-closure timeline under **Article XV-A**. Recurrence across partition cycles or rejoin cycles must be treated as a single open root-cause obligation, not as closure of each incident.

This section is an operational profile. It does not create rights and must not be read to narrow **Article XII-F**, **Chapter One §4.1**, or **Chapter Five** *Self-Healing*.
