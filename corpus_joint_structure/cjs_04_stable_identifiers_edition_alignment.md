...
2. When **CI**/**CF**/**CS** repeat the same joint interface paragraph, prefer a **one-line** pointer to **CJS-3** (including **CJS-3.7** for hybrid composition shared by **CI-9.1B.2** and **CF-2.5.2**) and keep operative checklists in the domain owner.
3. When **Cross-domain implementation layer** files repeat the same joint interface paragraph, apply **CJS-3.8** the same way: **one-line** pointer to **CJS-3** (or **CJS-2.2**), and keep **PRIM/PROT** operative substance in that layer (not duplicated at length in **CJS-3** OP blocks).
