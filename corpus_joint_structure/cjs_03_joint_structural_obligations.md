...
This section explains what must happen when more than one companion file applies to the same real-world situation. It prevents a common failure: every file looks satisfied on its own, but the combined system still blocks challenge, hides responsibility, weakens independence, or ignores the risk level assigned by classification.

Routing remains in **CJS-2.2** and `doc_architecture.md` section 4. These rules add to domain-owner rules in **CP**, **CS**, **CI**, and **CF**; they do not replace them.

### CJS-3.1 When joint obligations apply
Joint obligations apply when **CJS-2.2** says that a topic must be read with more than one companion, or when an adopting instrument says the **CJS folder** applies.

### CJS-3.2 No false partial compliance across companions
If two or more companions in a **CJS-2.2** chain impose structural duties on the same facts, compliance requires satisfying all material duties in that chain. A person or institution cannot satisfy one companion and then claim compliance while leaving another material companion duty unmet.

Emergency or temporary narrowing is allowed only when it is published, clearly bounded, time-limited, reversible where feasible, and traceable to the Sentient Constitution and companion emergency rules. Silent or indefinite "temporary" narrowing is non-compliant.
...
When an institution hosts, funds, administers, or technically runs forum infrastructure, the institution must make forum independence and contestability work in practice.

That includes budget, staffing, records, security, procurement, clerking, digital systems, and personnel systems. Independence on paper is not enough.

### CJS-3.5 Classification alignment for supervised scope
When an institution supervises systems governed by `corpus_systems.md` **Chapter S2** or **Chapter S3**, its published maps under **CI-9.1A** and related **CI-9** material must match the operative systems classification profile, including reclassification triggers.

It is non-compliant to use one label for institutional supervision and a different practical treatment in the systems companion when classification creates material duties.

### CJS-3.6 Implementation-label traceability and stricter-wins discipline
When **CS**, **CI**, or **CF** cite **PRIM** or **PROT** labels, adopters must keep those labels traceable and must not use companion text to weaken them.
...
This rule applies where **CJS-R01** or **CJS-R02** applies: delegated institutional bodies under **CI-9.1B**, and forum chambers, divisions, or panels under **CF-2.5** through **CF-2.5.4**.
...
Home-only or rotation-only structures are non-compliant for ordinarily binding bodies, except during a published emergency scope allowed by the domain owner's continuity rules. Emergency deviation must be limited to the emergency and must include a documented return to the hybrid baseline.
...
Where **CJS-3.7** applies, the instrument must publish:
...
- any class-, forum-, institution-, emergency-, or substitution-specific safeguards required by **CI-9.1B**, **CF-2.5**, **CF-11**, or related hooks.
...
### CJS-3.8 Implementation companion boundary (CP to CJS seam)
...
This rule applies where **CJS-R17** applies, or where a trust claim depends on more than one companion working together.
...
- the contest, escalation, evidence, and correction routes people can use; and
...
When a CJS abstraction and a domain-owner section seem to conflict, use this order:

1. The Sentient Constitution and core definitions, including **Chapter Fifteen**.
2. The canonical owner meaning in **CP**, **CS**, **CI**, or **CF**, as routed by **CJS-2.2** and `doc_architecture.md`.
3. The CJS Tier 1 abstraction for joint triggers and interlocks.
4. Subsection shorthand or drafting convenience text.

If ambiguity remains, do not treat the broader reading as authoritative. Route clarification to the canonical owner and update the CJS pointer.

### CJS-3.11 Class-scaled lane staffing and competency redundancy
This rule applies where **CJS-R18** applies: roles with materially binding effect under **Chapter Ten**, section **5**, where `corpus_systems.md` **Chapter S2** or **Chapter S3** scales the burden.
...
This subsection gives shared CJS terms for **CJS-R01** through **CJS-R04**. It does not replace **CI** or **CF** detail.
...
Owner detail remains in `corpus_institutions.md` including **CI-6**, **CI-8**, **CI-9.1B**, and `corpus_forum.md` including **CF-3**, **CF-4**, and **CF-5**.
