...
- GitHub Pages full render (assembled in CI on every deploy; not a second constitution) — [`.github/workflows/pages.yml`](.github/workflows/pages.yml), gated by `make pages-deploy-gate`; `make pages-site` is a local preview only and never needs regenerating as part of an edit. Full `make regression` runs on every push and pull request in `.github/workflows/checks.yml`
