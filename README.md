...
- Auditing / auditability / independent verification → **Article XV** (*Audit, Transparency, and Independent Verification*); Chapter Five [Auditability](core_05defs_oversight.md#auditability) and §3.2 cluster; implementation process home → **[CJS-5.3](corpus_joint_structure/cjs_05o_oversight_operations.md#cjs-53-audit-process-home)** (what / why / how / when); machine-facing terms → **CJS-5.3**–**CJS-5.5** (including **CJS-5.4** audit-process output disclosure preference: **Type O** where feasible, then qualified **Type G** / non-public source, then forensic or more-restricted tiers only as justified). Under the **oversight** Tetrad leg, oversight requires auditing; **Chapter Seven** system alignment certification is one especially large, high-stakes audit process among others — not the sole auditing home
