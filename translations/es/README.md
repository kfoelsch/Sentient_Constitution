# Edición en español — piloto (no vinculante)

<details>
<summary><strong><span style="color: #2563eb;">Ubicación en el corpus (no operativo): estructura del archivo y reglas de lectura</span></strong></summary>

> El contenido siguiente es **solo orientación para quien lee**. No añade, quita ni estrecha obligaciones vinculantes.
>
> Esta carpeta es un **piloto de idioma de lectura**. **No** es la Constitución Senciente. **No** puede cambiar los archivos numerados `core_*`. **No** es una edición de envío. Leerla **no** es [adoptar esta Constitución](../../core_16_amendment_ratification.md#10-ratification-and-adoption). Si una traducción y el original en inglés parecen discrepar, ganan los `core_*` numerados en inglés hasta que una edición posterior diga otra cosa. Véase [Reader-language editions](../../doc_architecture.md#reader-language-editions).
>
> **Fijado a la edición del corpus:** `SC-Corpus-2026.08.09` (vigente 2026-08-09; [README.md](../../README.md)). **Pre-lanzamiento.**

</details>

<br>

*En términos sencillos: esto es un borrador en español para medir costo y método, no un segundo texto constitucional.*

**Locale.** Español internacional (ustedes; sin voseo ni vosotros). No congela un país.

**Primer recorte, según arquitectura.** El orden de envío pide primero las guías de entrada fácil, la hoja del Piso de Derechos y la puerta pública. Este piloto traduce el [Preámbulo](core_00_preamble.md) y el [Capítulo Uno, Parte A](core_01_a_values_principles.md) a propósito, para medir cuánto cuesta un `core_*` numerado.

## Archivos

| Archivo | Original en inglés |
|---|---|
| [core_00_preamble.md](core_00_preamble.md) | [core_00_preamble.md](../../core_00_preamble.md) |
| [core_01_a_values_principles.md](core_01_a_values_principles.md) | [core_01_a_values_principles.md](../../core_01_a_values_principles.md) |
| [core_01_b_interaction_interpretation.md](core_01_b_interaction_interpretation.md) | [core_01_b_interaction_interpretation.md](../../core_01_b_interaction_interpretation.md) |
| [core_01_c_stewardship_capacity_principles.md](core_01_c_stewardship_capacity_principles.md) | [core_01_c_stewardship_capacity_principles.md](../../core_01_c_stewardship_capacity_principles.md) |
| [core_02_definition_structure.md](core_02_definition_structure.md) | [core_02_definition_structure.md](../../core_02_definition_structure.md) |
| [core_03_definition_integrity.md](core_03_definition_integrity.md) | [core_03_definition_integrity.md](../../core_03_definition_integrity.md) |
| [core_04_burden_traceability_verification.md](core_04_burden_traceability_verification.md) | [core_04_burden_traceability_verification.md](../../core_04_burden_traceability_verification.md) |
| [core_05__definitions_home.md](core_05__definitions_home.md) | [core_05__definitions_home.md](../../core_05__definitions_home.md) |
| [core_05_apex_accountability_leg.md](core_05_apex_accountability_leg.md) | [core_05_apex_accountability_leg.md](../../core_05_apex_accountability_leg.md) |
| [core_05_apex_continuity_aim.md](core_05_apex_continuity_aim.md) | [core_05_apex_continuity_aim.md](../../core_05_apex_continuity_aim.md) |
| [core_05_apex_flourishing_aim.md](core_05_apex_flourishing_aim.md) | [core_05_apex_flourishing_aim.md](../../core_05_apex_flourishing_aim.md) |
| [core_05_apex_oversight_leg.md](core_05_apex_oversight_leg.md) | [core_05_apex_oversight_leg.md](../../core_05_apex_oversight_leg.md) |
| [core_05_apex_participation_leg.md](core_05_apex_participation_leg.md) | [core_05_apex_participation_leg.md](../../core_05_apex_participation_leg.md) |
| [core_05_apex_timeliness_leg.md](core_05_apex_timeliness_leg.md) | [core_05_apex_timeliness_leg.md](../../core_05_apex_timeliness_leg.md) |
| [core_06_rights_part_a.md](core_06_rights_part_a.md) | [core_06_rights_part_a.md](../../core_06_rights_part_a.md) |
| [core_06_rights_part_b.md](core_06_rights_part_b.md) | [core_06_rights_part_b.md](../../core_06_rights_part_b.md) |
| [core_06_rights_part_c.md](core_06_rights_part_c.md) | [core_06_rights_part_c.md](../../core_06_rights_part_c.md) |
| [core_06_rights_part_d.md](core_06_rights_part_d.md) | [core_06_rights_part_d.md](../../core_06_rights_part_d.md) |
| [core_07_system_alignment_certification.md](core_07_system_alignment_certification.md) | [core_07_system_alignment_certification.md](../../core_08_system_alignment_certification.md) |
| [core_07_a_system_alignment_certification_evaluation.md](core_07_a_system_alignment_certification_evaluation.md) | [core_07_a_system_alignment_certification_evaluation.md](../../core_08_a_system_alignment_certification_evaluation.md) |
| [core_07_b_system_alignment_certification_record_process.md](core_07_b_system_alignment_certification_record_process.md) | [core_07_b_system_alignment_certification_record_process.md](../../core_08_b_system_alignment_certification_record_process.md) |
| [core_08_standing_assessment.md](core_08_standing_assessment.md) | [core_08_standing_assessment.md](../../core_09_standing_assessment.md) |
| [core_09_standing_integration.md](core_09_standing_integration.md) | [core_09_standing_integration.md](../../core_10_standing_integration.md) |
| [core_10_a_misconduct_designation.md](core_10_a_misconduct_designation.md) | [core_10_a_misconduct_designation.md](../../core_11_a_misconduct_designation.md) |
| [core_10_b_misconduct_pattern_applications.md](core_10_b_misconduct_pattern_applications.md) | [core_10_b_misconduct_pattern_applications.md](../../core_11_b_misconduct_pattern_applications.md) |
| [core_11_forum.md](core_11_forum.md) | [core_11_forum.md](../../core_12_forum.md) |
| [core_12_governance.md](core_12_governance.md) | [core_12_governance.md](../../core_13_governance.md) |
| [core_13_non_regression.md](core_13_non_regression.md) | [core_13_non_regression.md](../../core_14_non_regression.md) |

Los enlaces del piloto apuntan a los archivos en inglés que aún no tienen traducción. Los anclas HTML (`id`) se copian del original para poder comparar tramo a tramo.

## Glosario de este piloto

Términos de casa usados de forma estable en el español del piloto. No son definiciones; las definiciones siguen en el Capítulo Cinco en inglés.

| Inglés (fuente) | Español (piloto) | Nota |
|---|---|---|
| Sentient Constitution | Constitución Senciente | Nombre del instrumento |
| sentient / sentients | senciente / sencientes | Clase, no «personas» |
| Constitutional Tetrad | Tétrada Constitucional | participación · supervisión · rendición de cuentas · actuación a tiempo |
| Two Constitutional Aims | Dos Finalidades Constitucionales | Florecimiento y Continuidad |
| Flourishing | Florecimiento | |
| Continuity | Continuidad | |
| material stake | enjuego material | impacto, dependencia y riesgo |
| Rights Floor | Piso de Derechos | |
| oversight | supervisión | No «tribunal» ni «corte» |
| accountability | rendición de cuentas | |
| timeliness | actuación a tiempo | Un deber; no «oportunidad» en el sentido de chance |
| forum / forums / forum family | foro / foros / familia de foros | Nunca *tribunal* ni *corte* institucionales |
| standing records / measurement / effects / lock | registros / medición / efectos / bloqueo de trayectoria | No *legitimación procesal* |
| named pathway | vía nominada | |
| Constitutional Contract Layer | Capa del Contrato Constitucional | Quién puede gobernar |
| Stakeholder System Participation | Participación Sistémica de las Partes Afectadas | Voz dentro de sistemas ya autorizados |
| stewardship | administración responsable | Distinto de *custody* / custodia de edición |
| Due Process | Debido Proceso | |
| Contestability | impugnabilidad | |
| Key Practical Process Pipelines | Cadenas prácticas de proceso clave | |
| positive owner register | registro positivo de titulares | |
| Anti-Constitutional Trust Lock | Bloqueo de Confianza Anticonstitucional | |
| In plain terms | En términos sencillos | |

**Siguiente archivo:** [core_00_preamble.md](core_00_preamble.md)
