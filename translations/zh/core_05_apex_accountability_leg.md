<a id="accountability"></a>
<a id="chapter-five-definitions-accountability-tetrad-leg"></a>

# 第五章定义：问责（四元腿）

<details>
<summary><strong><span style="color: #2563eb;">在文本库中的位置（非操作性）：文件结构与阅读规则</span></strong></summary>

> 以下内容**仅为读者指引**。不增加、减少或收窄本文件或其他章节中有约束力的义务。
>
> 本文件是[英语第五章问责四元腿](../../core_05_apex_accountability_leg.md)的**读者语言试点**。**不是**感知者宪法的约束性部分。**不是**第二部宪法。**不是**发送版。**钉住** `SC-Corpus-2026.08.09`。若本译文与英语原文看似不一致，以编号文件 [`core_05_apex_accountability_leg.md`](../../core_05_apex_accountability_leg.md) 为准。阅读顺序与版本元数据保存在 [README.md](../../README.md)。方法与用语表：[translations/zh/README.md](README.md)。
>
> **上一篇（本语种）：** [core_05__definitions_home.md](core_05__definitions_home.md)
>
> **下一篇（本语言）：** [core_05_apex_continuity_aim.md](core_05_apex_continuity_aim.md)
> **阅读弧线：** O/M/A/C → 腿分解 → 测量族。

</details>

<details>
<summary><strong><span style="color: #2563eb;">溯源</span></strong></summary>

- 上游：[第五章指南针](core_05__definitions_home.md#chapter-five-compass-and-definition-map)；[宪法四元](core_00_preamble.md#constitutional-tetrad)（问责腿；按[实质利害](core_00_preamble.md#material-stake)缩放）；[两项宪法宗旨](core_00_preamble.md#two-constitutional-aims)；首要宗旨[繁盛](core_00_preamble.md#flourishing)。
- 下游：原则：[§10 尽责管理纪律下的治理](core_01_c_stewardship_capacity_principles.md#10-governance-under-stewardship-discipline)；[第一章 §11.6 继受责任与形式结构不得逃脱](core_01_c_stewardship_capacity_principles.md#116-successor-responsibility-and-formal-structure-non-escape)；[四元腿分解](#accountability-tetrad-leg-decomposition)；问责带叶定义在 [core_05_band_accountability.md](../../core_05_band_accountability.md)。
- 四元姐妹腿：[及时性](core_05_apex_timeliness_leg.md#timeliness-constitutional)。
- 一并阅读：[问责、可质疑性、裁决与争议解决、集体问责失败与不可抗力簇](../../core_05_band_integrative.md#accountability-contestability-and-collective-accountability-failure-cluster)、[可质疑性](../../core_05_band_accountability.md#contestability)、[可审计性](../../core_05_band_oversight.md#auditability)、[可归因行动](../../core_05_band_accountability.md#attributable-action-constitutional)、[归因完整性](../../core_05_band_accountability.md#attribution-integrity-constitutional)、[第一章 §11.6](core_01_c_stewardship_capacity_principles.md#116-successor-responsibility-and-formal-structure-non-escape)、[第三章 §2.1.1 形式标签与表征博弈](core_03_definition_integrity.md#211-formal-label-and-representation-gaming)，以及[第九章 §9.1](../../core_09_standing_integration.md#91-remediation-capacity-and-funding) / [§9.4](../../core_09_standing_integration.md#94-anti-evasion-and-look-through-authority)。

</details>

<br>

---

*用直白的话说：让行为者作答 — 归因、记录与真实救济途径必须起作用，而不只是纸面上的仪式。*

- O: 让行为者作答 — 本条目所点名的四元腿。
  - **范围内：** 实质性决定、失败与行为，可归因于可识别的行为者、角色或系统组件，并可通过审查、[救济](../../core_05_band_accountability.md#redress-and-remediation-constitutional)或与影响相称的纠正，向其追究作答。
  - **依赖于：** [可质疑性](../../core_05_band_accountability.md#contestability)、[可审计性](../../core_05_band_oversight.md#auditability)、[可归因行动](../../core_05_band_accountability.md#attributable-action-constitutional)、[归因完整性](../../core_05_band_accountability.md#attribution-integrity-constitutional)与[救济与补救](../../core_05_band_accountability.md#redress-and-remediation-constitutional) — 本腿要起作用所需的条件与通道。每一用语的详细范围与如何测量规则住在其自身定义上；从[四元腿分解](#accountability-tetrad-leg-decomposition)开始。
  - 范围外：由叶条目与 CJS 主责的叶定义细节、测量层级与实施机制。
<a id="accountability-m"></a>
- M: 如何判断应答义务在实践中是否真实。本行只指向那些测量；如何测量的规则坐落在每一项被链接的定义上。主题地图：[四元腿分解](#accountability-tetrad-leg-decomposition)。
<a id="accountability-a"></a>
- A: 在[宪法四元](core_00_preamble.md#constitutional-tetrad)下判断，按[实质利害](core_00_preamble.md#material-stake)缩放。问一问问责是否真实 — 归因链、记录与功能性救济途径在正常、降级与对抗性条件下起作用 — 而不只是仪式。
<a id="accountability-c"></a>
- C: 不合规当：
  - 实质伤害不可归因；
  - 在问责具有实质必要之处，问责路径不起作用或只是虚幻；
  - 标签或仪式实际上并不改变被治理的行为；
  - 归因只存在于形式上，而受影响方缺乏功能性[可质疑性](../../core_05_band_accountability.md#contestability)以启动审查、救济或纠正；
  - 经核实的义务仅因接管、重组、资产转让、解散或类似形式结构变更而消灭；或
  - 继受人或遗产安排使实质义务无人承担或实际上无法执行，而相称的转移或清偿本可行。

---

<a id="accountability-tetrad-leg-decomposition"></a>
<a id="accountability-leg-decomposition"></a>
<a id="tetrad-leg-decomposition"></a>

### 四元腿分解

*用直白的话说：本图列出问责簇与兄弟家园；叶定义继承本腿范围，除非它们另有说明。*

本腿下的叶定义继承其范围，除非它们明示收窄或放宽。完整主题地图：[第五章指南针](core_05__definitions_home.md#chapter-five-compass-and-definition-map)。

**依赖簇**（**Def.A1–Def.A4**）— 联合调用家园：

- **Def.A1 集体伤害边界、伤害，以及骚扰与欺凌** — [Def.A1 *集体伤害边界、伤害，以及骚扰与欺凌*](../../core_05_band_accountability.md#collective-harm-boundary-and-harm-cluster)
- **Def.A2 评议所族系与争议路由** — [Def.A2 *评议所族系与争议路由*](../../core_05_band_accountability.md#forum-families-and-dispute-routing-cluster)
- **Def.A3 轨迹状态、贡献与违规** — [Def.A3 *轨迹状态、贡献与违规*](../../core_05_band_accountability.md#standing-state-contribution-and-violation-cluster)
- **Def.A4 使用武力、自主强制、自主致命系统与大规模伤害武器** — [Def.A4 *使用武力、自主强制、自主致命系统与大规模伤害武器*](../../core_05_band_accountability.md#use-of-force-autonomous-coercion-and-mass-harm-cluster)

**四元姐妹腿：** [及时性](core_05_apex_timeliness_leg.md#timeliness-constitutional)。

**独立与半独立用语**住在[问责：独立用语](../../core_05_band_accountability.md#accountability-independent-terms)与[问责：半独立用语](../../core_05_band_accountability.md#accountability-semi-independent-terms)之下。

---

<a id="accountability-measurement-family"></a>
<a id="measuring-accountability"></a>

### 测量问责

<details>
<summary><strong><span style="color: #2563eb;">溯源</span></strong></summary>

- 一并阅读：[第一章 §11 激励对齐与系统俘获](core_01_c_stewardship_capacity_principles.md#11-incentive-alignment-and-system-capture)、[§13 市场结构](core_01_c_stewardship_capacity_principles.md#13-market-structure)，以及[第七章 §3.7 治理、激励与可质疑性纪律](../../core_07_a_system_alignment_certification_evaluation.md#37-governance-incentive-and-contestability-discipline)。

</details>

<br>

*用直白的话说：这些问题问奖惩、市场与应答义务是否让问责在实践中保持真实 — 而不只是纸面上存在。*

*问责测量族的第五章家园。序言 §2 把直白语言的类别问题保留在概览表中；族表、宪法用途与定义路由住在此处。*

*实质性*（[实质性认定](../../core_05_band_oversight.md#materiality-determination)）是整合性的 — 不是单独的测量类别；它在[实质利害](core_00_preamble.md#material-stake)下缩放本族适用的强度。

| 测量族 | 直白问题 | 主要宪法用途 |
|---|---|---|
| 激励对齐与替代指标完整性 | 奖惩与度量是否跟踪真实宪法结果？ | 检测扭曲激励、俘获与[替代指标偏离](../../core_05_band_oversight.md#proxy-divergence) |
| 市场结构与可竞争性 | 进入、退出、议价与替代选择能否保持真实？ | 在能力主张依赖于开放市场之处，适用反集中与反支配纪律 |

这些测量问奖惩结构、市场力量与应答义务是否让[宪法四元](core_00_preamble.md#constitutional-tetrad)的**问责**腿在实践中保持真实 — 而不只是纸面上存在。

操作性测量层级住在第五章定义家园上：

- [激励对齐](../../core_05_band_integrative.md#incentive-alignment)
- [替代指标偏离](../../core_05_band_oversight.md#proxy-divergence)
- [可质疑性](../../core_05_band_accountability.md#contestability)
- [市场结构](../../core_05_band_accountability.md#market-structure-constitutional)
- [市场集中阈值](../../core_05_band_accountability.md#market-concentration-threshold-constitutional)
- [受保护报告（揭发）](../../core_05_band_accountability.md#protected-reporting-whistleblowing)
- [问责](#accountability)（仅链接的腿头汇总）


---

**上一文件：** [core_05__definitions_home.md](core_05__definitions_home.md)

**下一文件（本语言）：** [core_05_apex_continuity_aim.md](core_05_apex_continuity_aim.md)

**有约束力的原文：** [core_05_apex_accountability_leg.md](../../core_05_apex_accountability_leg.md)
