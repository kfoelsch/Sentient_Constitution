<a id="chapter-seven-system-alignment-certification-index"></a>
<a id="chapter-seven-system-alignment-certification-reading-index"></a>
# BAB TUJUH: SERTIFIKASI KESELARASAN SISTEM (INDEKS BACA)

<details>
<summary><strong><span style="color: #2563eb;">Letak dalam korpus (non-operatif): struktur berkas dan aturan baca</span></strong></summary>

> Isi berikut **hanya panduan pembaca**. Tidak menambah, mengurangi, atau mempersempit kewajiban yang mengikat di berkas ini atau di bab lain.
>
> Berkas ini adalah **uji coba bahasa pembaca** atas [indeks baca Bab Tujuh bahasa Inggris](../../core_08_system_alignment_certification.md). **Bukan** bagian mengikat Konstitusi Makhluk Sadar. **Bukan** konstitusi kedua. **Bukan** edisi kirim. **Disematkan** pada `SC-Corpus-2026.08.09`. Jika terjemahan ini dan sumber bahasa Inggris tampak berselisih, berkas bernomor [`core_07_system_alignment_certification.md`](../../core_08_system_alignment_certification.md) yang menang. Urutan baca dan metadata edisi tetap di [README.md](../../README.md). Metode dan glosarium: [translations/id/README.md](README.md).
>
> **Bab Tujuh** terbagi di dua berkas mengikat yang dibaca sebagai satu instrumen:
>
> | Bagian | Berkas | Mengatur |
> |------|------|------|
> | **Bagian A — Evaluasi** | [`core_07_a_system_alignment_certification_evaluation.md`](core_07_a_system_alignment_certification_evaluation.md) | §1–§10: tujuan, kelas sistem, evaluasi seluruh-sistem (termasuk cakupan piagam versus batas fungsional), kait evaluasi ranah, penelusuran kelas yang bersifat ilustratif |
> | **Bagian B — Catatan dan proses** | [`core_07_b_system_alignment_certification_record_process.md`](core_07_b_system_alignment_certification_record_process.md) | §11–§16: catatan sertifikasi (termasuk medan [Piagam](core_05_band_continuity.md#charter) yang mengatur), transparansi/dapat-digugat, peran forum, urutan supervisi, jembatan jejak, pembukaan ulang — penomoran bab berlanjut dari Bagian A §10 |
>
> Jangkar warisan dari Bab Tujuh bekas berkas-tunggal dipertahankan pada berkas Bagian A dan Bagian B. Tautan generik **Bab Tujuh** di berkas lain diarahkan ke bagian yang sesuai menurut nomor bagian.
> Pembentukan Piagam kelembagaan, medan minimum, dan kewajiban tinjauan berkala hidup di [`corpus_institutions`](../../corpus_institutions/ci_03_institutional_design_separation_of_powers.md) **CI-3.6** — Bab Tujuh memakai medan itu; ia tidak mendefinisikannya ulang.

>
> **Sebelumnya (lokal ini):** [core_06_rights_part_d.md](core_06_rights_part_d.md)
>
> **Berikutnya (bahasa ini):** [core_07_a_system_alignment_certification_evaluation.md](core_07_a_system_alignment_certification_evaluation.md#chapter-seven-part-a-certification-evaluation)
> **Alur baca:** indeks baca → Bagian A evaluasi (§1–§10) → Bagian B catatan dan proses (§11–§16)

</details>

<br>

*Dalam bahasa sederhana: Bab Tujuh terbagi dua — Bagian A untuk cara mengevaluasi, Bagian B untuk catatan dan proses. Bacalah keduanya sebagai satu bab.*

Baca **Bagian A** dulu untuk persyaratan evaluasi; **Bagian B** untuk catatan, proses forum, dan jembatan jejak.
Di bawah kaki Tetrad **pengawasan**, pengawasan menuntut audit; Bab Tujuh adalah satu proses audit yang sangat besar dan bertaruhan tinggi di antara yang lain (tinjauan keselarasan yang disupervisi forum dengan hasil pengakuan). Lantai audit tetap pada **Pasal XV** dan [Dapat diaudit](core_05_band_oversight.md#auditability) Bab Lima.
- [Bab Tujuh, Bagian A — Evaluasi](core_07_a_system_alignment_certification_evaluation.md#chapter-seven-part-a-certification-evaluation)
- [Bab Tujuh, Bagian B — Catatan dan Proses](core_07_b_system_alignment_certification_record_process.md#chapter-seven-part-b-certification-record-and-process)
---

**Berkas sebelumnya:** [core_06_rights_part_d.md](core_06_rights_part_d.md)

**Berkas berikutnya (bahasa ini):** [core_07_a_system_alignment_certification_evaluation.md](core_07_a_system_alignment_certification_evaluation.md#chapter-seven-part-a-certification-evaluation)

**Sumber mengikat:** [core_07_system_alignment_certification.md](../../core_08_system_alignment_certification.md)
