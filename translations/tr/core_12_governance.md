<a id="chapter-twelve-constitutional-contract-legitimacy-authorization-and-stewardship"></a>
# ON İKİNCİ BÖLÜM: ANAYASAL SÖZLEŞME, MEŞRUİYET, YETKİLENDİRME VE SORUMLU YÖNETİM

<details>
<summary><strong><span style="color: #2563eb;">Corpus konumu (işlemsel değil): dosya yapısı ve okuma kuralları</span></strong></summary>

> Aşağıdaki içerik **yalnızca okur rehberidir**. Bu dosyada ya da başka bölümlerde bağlayıcı yükümlülükleri eklemez, çıkarmaz ya da daraltmaz.
>
> Bu dosya, [İngilizce On İkinci Bölüm](../../core_12_governance.md)ün bir **okur-dili pilotudur**. Algılayanlar Anayasasının bağlayıcı bir parçası **değildir**. İkinci bir anayasa **değildir**. Bir gönderim baskısı **değildir**. `SC-Corpus-2026.08.09`a **sabitlenmiştir**. Bu çeviri ile İngilizce asıl görünür biçimde ayrışırsa, numaralı [`core_12_governance.md`](../../core_12_governance.md) kazanır. Okuma sırası ve baskı üstverisi [README.md](../../README.md)de tutulur. Yöntem ve sözlük: [translations/tr/README.md](README.md).
>
> **On İkinci Bölüm**ü içerir — **Anayasal Sözleşme Katmanı**. Yönetme yetkisinin kendisi için anayasal meşruiyet ve sorumlu yönetim gerekleri; zaten yetkilendirilmiş sistemlerdeki **Etkilenen Tarafların Sistem Katılımı**ndan ayrıdır ([Önsöz](core_00_preamble.md#chapter-00-preamble--foundational-requirements)). Değişiklik ve benimseme geçerlilik denetimleri [On Üçüncü Bölümden On Beşinci Bölüme](../../core_13_non_regression.md#chapter-thirteen-non-regression-and-substantive-amendment-validity) sürer. Bölüm numaralandırması ve çapraz gönderimler bütünleşik belgelerle örtüşür.


>
> **Önceki (hâlâ İngilizce):** [core_11_forum.md](core_11_forum.md#chapter-eleven-forums-and-jurisdiction)
>
> **Sonraki (hâlâ İngilizce):** [core_13_non_regression.md](../../core_13_non_regression.md#chapter-thirteen-non-regression-and-substantive-amendment-validity)
> **Okuma yayı:** §0 katman sınırı → §1 yetkilendirme/meşruiyet → §1.1 düzenek aileleri → §1.2 elverişlilik/demokratik asgariler → §1.3 geri çağırma sınıfı → §2 bütünlük kültürü → §3 sorumlu yönetim yönü → §4 oylama/kolektif seçim → §5 yetkilendirilmiş roller
</details>

<details>
<summary><strong><span style="color: #2563eb;">Okur rehberi (işlemsel değil): bu bölümün nerede yaşadığı ve burada neyin kaldığı</span></strong></summary>

> Aşağıdaki içerik **yalnızca okur rehberidir**. Bu bölümde ya da başka bölümlerde bağlayıcı yükümlülükleri eklemez, çıkarmaz ya da daraltmaz.
>
> Nerede yaşar (gezinme):
> - **Anayasal sahip:** bu bölüm yönetme yetkisi için meşruiyet ve sorumlu yönetim gereklerini tanımlar.
> - **Uygulama sahibi:** belirlenmiş uygulama katmanları usule ilişkin gözetim mekaniğini, kadro tasarımını, işlemsel denetimleri ve belirlenmiş kurumsal mekaniği sağlar.
> - **Yer-değiştirmeme kuralı:** anayasal meşruiyet eşikleri burada kalır; iş akışı ayrıntısı sahip katmanlara aittir.
>
> **Mimari — *Yalın dille* yerleşimi:** Her *Yalın dille* satırı Altıncı Bölümle aynı düzeni kullanır: İz gezinme bloğunun (ve onu izleyen satır sonunun) hemen ardından, o kesit ya da alt kesitin işlemsel paragraflarından ve madde işaretli listelerinden **önce** görünür. Bir alt kesit listeden önce giriş cümleleriyle açıldığında (örneğin düzenek-ailesi girişinden sonra **§§ 1.1–1.2**, **Beşinci Bölüm** sorumlu yönetim tanım dizini maddelerinden önce kesit 3 ve eşitlik tabanı maddelerinden önce kesit 4.1), açıklama listenin ya da en yakından izlediği bloğun hemen önüne oturur. Yalnızca okura dönük açıklama; bağlayıcı metni eklemez, çıkarmaz ya da daraltmaz.

</details>

<br>

On İkinci Bölüm, **anayasal-sözleşme meşruiyeti, sorumlu yönetim yönü, yoğunlaşma ve demokratik denetimlerin** anayasal sahibidir.

Bu bölüm **Anayasal Sözleşme Katmanı**nı belirtir: yönetme yetkisinin kendisi için anayasal meşruiyet ve sorumlu yönetim gerekleri. Kim yönetebilir, yönetme gücüne ne izin verir ve hangi sorumlu yönetim karakterinin sürdürülmesi gerektiğini tanımlar.

<a id="0-layer-boundary-for-this-chapter"></a>
### 0. Bu bölümün katman sınırı
<details>
<summary><strong><span style="color: #2563eb;">İz</span></strong></summary>

- Yukarı: İlkeler: [Birinci Bölüm](core_01_c_stewardship_capacity_principles.md#chapter-01-principles-and-constraints) bütünleşik kısıt yığını; [Altıncı Bölüm](core_06_rights_part_a.md#chapter-six-foundational-rights) Temel Hak Tabanı [D Kısmı](core_06_rights_part_d.md)na kadar.
- Aşağı: [§§ 1–5](#1-authorization-and-legitimacy-of-governing-authority) (bu bölümdeki sorumlu yönetim ve kolektif-seçim gerekleri); [Sekizinci Bölüm](core_08_standing_assessment.md#chapter-eight-compliance-violation-and-standing-model) genel **katkı, ihlal ve güzergâh** ölçümü (Katkı Ekseni ve İhlal Ekseni), her sayısal etki yuvası dahil; [Onuncu Bölüm](core_10_a_misconduct_designation.md#chapter-ten-anti-constitutional-misconduct) nitelikli **İhlal Ekseni s = 7, 8 ya da 9** bulguları için son anayasa karşıtı-kötü-davranış ataması.
- Birlikte oku: [İkinci Bölümden Dördüncü Bölüme](core_02_definition_structure.md) (bütünlük, iz, doğrulama); [Beşinci Bölüm](core_05__definitions_home.md#chapter-five-foundational-definitions) (kanonik tanımlar); [*Yönetişim mimarisi, gözetim, bağımlılık, merkezsizleşme, yoğunlaşma, piyasa yapısı ve çıkış yolu bütünlüğü*](../../core_05_band_accountability.md#governance-architecture-oversight-decentralization-and-concentration-cluster) (yetki yerleşimi, gözetim tasarımı, bağımlılık, merkezsizleşme, yoğunlaşma, teşvik yönlendirmesi ya da çıkış-yolu çözümlemesinin uyumu maddeten etkilediği yerde ortak-çağrı kümesi).

</details>

<br>

*Yalın dille: bu bölüm yönetme gücü için anayasal sözleşmeyi belirtir — kural koymanın meşru sayılması ve sorumlu yönetimin anayasal sayılması için neyin doğru olması gerekir. Asgari taban burada yaşar; işletim ayrıntısının çoğu uygulama katmanlarında yaşar.*

- Anayasal gerekleri koyar, tam işletim ayrıntısını değil.
- Katılım, usul güvencesi ve bağlayıcı-benimseme kapıları Altıncı Bölümde demirli kalır (özellikle **Madde XI-A** (*Etkilenen Tarafların Sistem Katılımı ve temsili*)).
- **Sekizinci Bölüm** altında **genel** güzergâh ölçümü (Katkı Ekseni ve İhlal Ekseni — katkı **durumu**, ihlal niteliği ve her sayısal etki yuvası) **Sekizinci Bölüm**de demirli kalır. Nitelikli **İhlal Ekseni s = 7, 8 ya da 9** bulguları için **son anayasa karşıtı-kötü-davranış ataması** **Onuncu Bölüm**de demirli kalır; Onuncu Bölüm sayısal yuvayı atamaz ya da taşımaz.
- Ayrıntılı usul, kadro, oylama uygulaması ve yürütme protokolleri belirlenmiş uygulama katmanlarınındır.

<a id="1-authorization-and-legitimacy-of-governing-authority"></a>
### 1. Yönetme yetkisinin yetkilendirilmesi ve meşruiyeti
<details>
<summary><strong><span style="color: #2563eb;">İz</span></strong></summary>

- Yukarı: İlkeler: [Birinci Bölüm §5 Özgürlük](core_01_a_values_principles.md#5-freedom-bounded-agency), [§9 Sorumlu yönetim ve dağıtılmış anlayış](core_01_c_stewardship_capacity_principles.md#9-stewardship-and-distributed-understanding), [5. Süreç çatışması çözümü](#6-process-conflict-resolution); [On İkinci Bölüm §0](#0-layer-boundary-for-this-chapter).
- Alt kesitler: [§1.1](#11-mechanism-families-auditability-and-pluralism) (*Düzenek aileleri, denetlenebilirlik ve çoğulculuk*); [§1.2](#12-eligibility-contested-selection-and-democratic-minimums) (*Elverişlilik, yarışmalı seçim ve demokratik asgariler*); [§1.3](#13-recall-class-pathways-and-mid-cycle-transfer-guardrails) (*Geri çağırma sınıfı adlandırılmış yollar ve dönem-içi devir korkulukları*).
- Aşağı: [§§ 2–5](#2-ethical-culture-and-integrity-federated-scale) altında bütünlük, strateji, oylama ve rol ödevleri; [On Beşinci Bölüm](../../core_15_amendment_ratification.md#chapter-fifteen-amendment-ratification-and-procedural-validity) altında benimseme ve onaylama; Altıncı Bölümde meşruiyet, katılım ve itiraz edilebilirlik — özellikle [Madde XI-C: Meşruiyet kapısı ve simgesel katılım karşıtı](../../core_06_rights_part_b.md#article-xi-c-legitimacy-gate-and-anti-token-participation), [Madde XVIII: Güzergâh ve katılım statüsü](../../core_06_rights_part_c.md#article-xviii-standing-and-participation-status) ve aşağıdaki işlemsel maddelerde anahtarlanan ifade, toplanma ve usul adilliği maddeleri.
- Birlikte oku: Bu kesitte adlandırılan belirlenmiş **corpus** ve **[corpus_systems.md](../../corpus_systems.md)** kancaları ([corpus_systems.md](../../corpus_systems.md), [corpus_institutions.md](../../corpus_institutions.md)).

</details>

<br>

**Yönetme yetkisi**, bu Anayasa altında algılayanları maddeten etkileyen kararları alma ya da yaptırma gücüdür.

Benimseyenin seçtiği ve yayımladığı **belgelenmiş bir meşruiyet düzeneği** üzerine oturmalıdır.

Düzenek aileleri, yayımlama, denetlenebilirlik ve çoğulculuk **§1.1**de ele alınır; yönetme rolleri için elverişlilik ve demokratik-kurum asgari denetimleri **§1.2**de; geri çağırma sınıfı dönem-içi devir korkulukları **§1.3**te.

<a id="11-mechanism-families-auditability-and-pluralism"></a>
#### 1.1 Düzenek aileleri, denetlenebilirlik ve çoğulculuk

*Yalın dille: kim yönetirse gücün nasıl yetkilendirildiğine dair açık, seçilmiş bir kural göstermelidir — seçimler, kura, federasyon oyları, **antlaşma, pakt ya da berat** süreçleri ya da melezler — resmiyetsiz bir omuz silkme değil. Yürürlükteki kurallar görünür ve **doğrulamaya açık** olmalıdır; sessizlik, salt kullanım ya da marka rıza sayılmaz. Yetkilendirme çürüdüğünde ya da ele geçirildiğinde **algılayanların** onu gerçekten itiraz edebileceği ve mümkün olduğunca gücü barışçıl yenileyebileceği ya da değiştirebileceği yollara ihtiyacı vardır — ve haklar ile tahakküm-yokluğu duruyorsa farklı meşru düzenlemeler bir arada yaşayabilir.*

**Düzenek aileleri**

- temsili seçimler ya da rıza yenileme
- olağan seçim ya da yeniden yetkilendirme olayları arasında **süregelen vekâletler** için **topluluk-başlatımlı erken devir ya da yeniden yetkilendirme** (*geri çağırma sınıfı* adlandırılmış yollar); **§1.3** yönetir
- belirlenmiş heyetler için **kura** ya da yurttaş piyangosu
- yayımlanmış kurallar altında ağ üyeleri ya da üye organlarca **federatif onaylama**
- kayda geçmiş rıza ile **antlaşma, pakt ya da berat** benimseme
- **yüksek etkili** kararlar için **Madde XI-C** (*Meşruiyet kapısı ve simgesel katılım karşıtı*) meşruiyet kapılarını karşılayan melez ya da sıralı tasarımlar

**Kura bütünlüğü ve ele geçirme karşıtı kullanım**

- **Ele geçirme karşıtı işlev:** Kura, rastgele seçimin öngörülebilir ele geçirmeyi maddeten azalttığı yerde belirlenmiş yönetme, gözetim, inceleme, gündem-koyma, tedarik, denetim ya da anayasal-müzakere heyetleri için yolsuzluk karşıtı ve ele geçirme karşıtı bir düzenek olarak kullanılabilir:
  - kampanya finansmanı;
  - mevcut-görev bağımlılığı;
  - kayırmacılık;
  - hizip kapıcılığı; ya da
  - ön-seçim rüşveti.
- **Bütünlük tabanı:** **Yönetme yetkisini** kullanan ya da maddeten şekillendiren bir kura düzeneği şunları yayımlamalıdır:
  - denetlenebilir seçim kuralları;
  - elverişlilik sınırları;
  - dışlama gerekçeleri;
  - yerine koyma kuralları;
  - rotasyon temposu;
  - tazminat ve erişim destekleri;
  - çıkar çatışması denetimleri;
  - brifing ve kanıt kuralları;
  - kolaylaştırıcı ya da kadro bağımsızlık denetimleri;
  - lobi karşıtı ve sindirme karşıtı korumalar; ve
  - şu itiraz yolları:
    - seçim başarısızlığı;
    - manipülasyon;
    - ele geçirme; ya da
    - usul çarpıtması.
- **Meşruiyet aklama yok:** Bir yurttaş piyangosu, yurttaş meclisi ya da rastgele seçilmiş heyet, ilgili bir yetkinin anlamlı müzakereyi, itiraz edilebilirliği ya da **Madde XI-C** (*Meşruiyet kapısı ve simgesel katılım karşıtı*) katılım kapılarını bozacak biçimde şunları denetlediği kararlar için meşruiyet tiyatrosu olarak kullanılmamalıdır:
  - gündem;
  - kanıt kaydı;
  - kolaylaştırıcı denetimi;
  - uygulama yanıtı; ya da
  - kabul ölçütleri.
- **Uygulama sahibi:** Kura bütünlüğü için işlemsel korumalar belirlenmiş uygulama metninde ve **corpus_institutions.md** **CI-5** (*Çatışma bütünlüğü, ele geçirme karşıtı ve yolsuzluk karşıtı*) ele geçirme karşıtı ve yolsuzluk karşıtı denetimlerinde yaşar. Bu korumalar şunları içerir:
  - rastgeleleştirme doğrulaması;
  - çıkar taraması;
  - kadro ayrımı;
  - açıklama;
  - korumalı yükseltme; ve
  - ele geçirme iddialarına yanıt.

**Denetlenebilirlik, yayımlama ve gasp etmeme**

- **Yayımlama ve orantılılık:** Yürürlükteki düzenek denetlenebilir olmalıdır. Denetim kayıtları şunları içermelidir:
  - kapsam;
  - yenileme ya da yeniden yetkilendirme temposu; ve
  - devredilmiş gücün maddi sınırları.
  Denetlenebilirlik **[corpus_systems.md](../../corpus_systems.md)**, CS-3 — Sistem sınıflaması ve ele alma ve Beşinci Bölüm altında **maddi etki** ile orantılı olmalıdır ([*Maddi*](core_05_band_oversight.md#material), [*Bağımlılık*](../../core_05_band_continuity.md#dependency), [*Etkilenen taraf*](../../core_05_band_participation.md#stakeholder)).
- **Gasp etmeme:** Yönetme organları sessizliği, salt kullanımı ya da **pazarlama hizasını** yetkilendirme yerine koyamaz. Kasıtlı benimseme yine **On Altıncı Bölüm** (*Onaylama ve benimseme*) tarafından yönetilir.

**İtiraz, düzeltme ve çoğulculuk**

- **Başarısızlık ve düzeltme:** Yetkilendirme itiraz edildiğinde, maddeten yanlış hizalandığında ya da ele geçirildiğinde sistemler **itiraz edilebilir adlandırılmış yollar** sağlamalıdır. Adlandırılmış yollar şunları içerir:
  - **Madde XI-C** (*Meşruiyet kapısı ve simgesel katılım karşıtı*);
  - **Madde XII-B** (*İtiraz, inceleme ve giderim hakkı*);
  - **Madde XXIII-A** (*Adalet amacı ve kapsamı*); ve
  - benimsenmiş uygulama metnindeki uygulanabilir usul-bütünlüğü korumaları.
  Elverdiğinde sistemler, gücün incelenemez sürekliliği yerine **barışçıl ardıllık ya da yeniden yetkilendirme** sağlamalıdır.
- **Çoğulculuk:** Bu Anayasa tek bir küresel siyasal yapı **zorunlu kılmaz**. Sınırlar, **tahakküm-yokluğu** ve Altıncı Bölüm Hak Tabanları karşılandığı sürece federasyonlar boyunca birden çok düzenek bir arada yaşayabilir.

<a id="12-eligibility-contested-selection-and-democratic-minimums"></a>
#### 1.2 Elverişlilik, yarışmalı seçim ve demokratik asgariler

*Yalın dille: birini yönetme ya da kritik sorumlu yönetim rollerinden bir doğum günü kuralıyla barındıramazsınız — uygunluk ve hazırlık denetime dayanan yollarla gösterilmeli, yaş diye kaçırılmamalıdır. Gerçek bir meşruiyet düzeneği gerçek seçenekler sunar, itirazın işe yarayacağı kadar muhalefeti korur ve gücü devretmek için inandırıcı bir yol içerir — kurulu bir yarış değil, muhalefete misilleme değil, usul kılığına girmiş kalıcı mevcut görev değil.*

**Yönetme ve kritik-sorumlu-yönetim rolleri için elverişlilik**

- **Yaş gerekleri olmadan elverişlilik:**
    - Bu bölüm altında **yönetme yetkisi** kullanan roller elverişlilik koşulu olarak **takvim yaşı** ya da **yaşam-evresi tavanı** koymaz.
    - Aynı yasak **[corpus_systems.md](../../corpus_systems.md), CS-4 — Kritik sistem sorumlu yönetimi**nde tanımlanan **Kritik Sistem Sorumlu Yönetimi** altındaki rollere de uygulanır.
    - **Yetkinlik**, **ardıllık hazırlığı** ve **uygunluk** standartları yine **On İkinci Bölüm**, kesit **5** (*Yetkilendirilmiş roller, yetkinlik geliştirme ve katkı*), **[corpus_systems.md](../../corpus_systems.md), CS-4 — Kritik sistem sorumlu yönetimi**, belirlenmiş uygulama metni ve **corpus_institutions.md** (ör. **CI-4** (*Atama, yetkinlik, rotasyon ve görevden alma*) ve ilgili kesitler altında atama ve rol-yolu mekaniği) tarafından, uygulanabilir olduğunda yönetilir.
    - Bu standartlar **İkinci Bölümden Dördüncü Bölüme** altında **gösterilmeli**, Beşinci Bölümde [**Maddi adillik**](../../core_05_band_participation.md#substantive-fairness-constitutional), [**Orantılılık**](../../core_05_band_accountability.md#proportionality) ve [**Gereklilik**](../../core_05_band_accountability.md#necessity)e tabi olmalı ve yaşa dayalı dışlamanın **vekilleri** olarak kullanılmamalıdır.

**Yarışmalı seçim, muhalefet koruması ve barışçıl ardıllık**

- **Demokratik-kurum asgari denetimleri:** Yönetme yetkisini yetkilendirdiğini iddia eden herhangi bir meşruiyet düzeneği asgari olarak şunların tümünü sağlamalıdır:

    1. **Yarışmalı seçim** — alternatif adayların, önerilerin ya da konumların yetkilendiren toplulukça sunulup değerlendirilmesi için anlamlı fırsat.
    2. **Muhalefet koruması** — muhalif algılayanların, önerilerin ve koalisyonların misilleme olmadan örgütlenmesi, iletişim kurması ve mevcut yetkiye itiraz etmesi için gerçek, salt biçimsel olmayan özgürlük; **Madde V-H** (*İfade, toplanma ve basın*), **Madde IX-A** (*Manipülasyondan özgürlük*) ve **Madde XIII-A** (*Korumalı faaliyet*) ile tutarlı.
    3. **Barışçıl-ardıllık güvencesi** — düzeneğin kendi belirttiği koşullarda yetkinin devri ya da yeniden yetkilendirilmesi için tanımlanmış bir adlandırılmış yol; barışçıl-ardıllık adlandırılmış yolunun reddi ya da maddi bozulması yukarıdaki **Başarısızlık ve düzeltme** altında düzeneğin başarısızlığı sayılır.

    - Bu denetimler [**Temel anayasal seçim**](core_05_band_integrative.md#foundational-constitutional-choice)e (Beşinci Bölüm) ve düzeneğin kapsamındaki başka herhangi bir yetkilendirme eylemine uygulanır.
    - Denetimler **düzenek-yansız**dır: tek bir siyasal tür zorunlu kılmaz ve temsili, kura, federatif-onaylama, **antlaşma, pakt ya da berat** tasarımları ve melez tasarımlar boyunca uygulanır.
    - Gizli bilgi-küresi bağımlılığı, **Madde XII-E** (*Yüksek-özerklik sistemleri ve araç-aracılı süreç bütünlüğü*) özerklik-sorumlu-yönetim ele geçirmesi ya da karşılaştırılabilir yapısal yollarla işlevsel dışlamayla eşlenen nominal muhalefet koruması hem bu denetim hem **Çoğulculuk** hükmü altında uyumsuzluktur.

**Forum atama bağımsızlık tabanı**

- [**Bütünlük**](core_11_forum.md#45-integrity-forums), [**Anayasal**](core_11_forum.md#46-constitutional-forums) ve [**Çevre**](core_11_forum.md#44-environment-forums) forumları ve algılama-statüsü hükme bağlamasını dinlediklerinde [**Teknik Forum Alanları**](core_11_forum.md#42-technical-forum-domains) **yayımlanmış**, **yarışmalı**, **döndürülebilir** atama ya da eşdeğer bir bağımsızlık denetimi (kura, federatif atama ya da karşılaştırılabilir) yoluyla kadrolanmalıdır.
- Bu taban **düzenek-yansız**dır: tek bir siyasal tür ya da tek bir atama yöntemi zorunlu kılmaz.
- O heyetleri [Dokuzuncu Bölüm §9.2](core_09_standing_integration.md#92-remedy-system-durability) çare-sistemi dayanıklılık tabanına atamamak ya da finanse etmemek bir meşruiyet ve çare başarısızlığıdır, uygulamanın açık bırakabileceği bir boşluk değil.
- Atama, yetkinlik, rotasyon ve görevden alma mekaniği [**CI-4**](../../corpus_institutions/ci_04_appointment_competency_rotation_removal.md) (*Atama, yetkinlik, rotasyon ve görevden alma*) tarafından yönetilir; **CI-4** bu tabanı daraltmamalıdır.

<a id="13-recall-class-pathways-and-mid-cycle-transfer-guardrails"></a>
#### 1.3 Geri çağırma sınıfı adlandırılmış yollar ve dönem-içi devir korkulukları

*Yalın dille: güç gerçek oylar arasında bir dönem ya da aralıkta yürürse yazılı kurallar topluluğun onu erken bitirip bitiremeyeceğini söylemeli — ve seçimden bir hafta sonraki bir dilekçe, akut ve iyi belgelenmiş bir şey daha hızlı bir izi haklı kılmadıkça yıllar sonrakinden daha yüksek bir eşiğe çarpmalıdır.*

**Kapsam**

- Bu alt kesit, **belgelenmiş bir meşruiyet düzeneğinin** düzeneğin olağan **yarışmalı seçim** olayları ya da karşılaştırılabilir **yeniden yetkilendirme** kilometre taşları arasında tanımlı bir aralık boyunca süren **bireysel ya da kurul vekâletleri** yoluyla **yönetme yetkisini** yetkilendirdiği yerde uygulanır.
- Vekâletlerin o şekilde işlemediği tasarımlar (birçok **kura** heyeti, **kısa-rotasyon** organları ya da **yalnızca-antlaşma** yetkilendirmesi dahil) için geri çağırma sınıfı adlandırılmış yollar **istemez**; yeter ki **§1.2**deki **barışçıl-ardıllık güvencesi** düzeneğin kendi koşullarında karşılanmış kalsın.

**Geri çağırma sınıfı adlandırılmış yol gereği**

- Yukarıdaki kapsam içinde yürürlükteki düzenek, aşağıdaki korkulukları karşılayan ve dönem-içi devir için **barışçıl-ardıllık güvencesini** **§1.1**deki **Başarısızlık ve düzeltme** ile birlikte okuyarak işleyen bir **topluluk-başlatımlı erken devir ya da yeniden yetkilendirme** adlandırılmış yolu (yaygın olarak **geri çağırma**, **güvensizlik oyu** ya da eşdeğeri diye etiketlenir) yayımlamalıdır.

**Korkuluklar**

- **Zamansal orantılılık (dilekçe ve usul):** Geri çağırma sınıfı önlemlerin başlatma ve tamamlama gerekleri, vekâletin düzeneğin olağan **yarışmalı seçimi** ya da **Temel anayasal seçim** yoluyla en son yetkilendirilmesinden ya da karşılaştırılabilir koşullarda **olumlu yenilenmesinden** bu yana **geçen süreyle orantılı** olmalıdır. Bu gerekler şunları içerir:
  - dilekçe ya da sponsor eşikleri;
  - imza pencereleri;
  - yeter sayılar;
  - oylama takvimleri; ve
  - herhangi bir nitelikli çoğunluk ya da çok-adımlı onay.
  Daha kısa geçen aralıklar **belgelenmiş akut başarısızlık** yoksa **daha sıkı** gerekler koymalıdır; akut başarısızlık şunları içerir:
  - yetkilendirmenin maddi yanlış hizalanması ya da ele geçirilmesi;
  - barışçıl-ardıllık adlandırılmış yolunun reddi; ya da
  - **Altıncı Bölüm** ve benimsenmiş uygulama usulü yoluyla ele alınan **Hak Tabanı** tehdidi.
  O saate bağlanan yükselen kademeler, **Birinci Bölüm** ve **İkinci Bölümden Beşinci Bölüme** altında [**Maddi adillik**](../../core_05_band_participation.md#substantive-fairness-constitutional), [**Gereklilik**](../../core_05_band_accountability.md#necessity) ve [**Orantılılık**](../../core_05_band_accountability.md#proportionality)ı karşıladıklarında belirlenmiş uygulama metninde izinlidir.
- **Usul adilliği:** Geri çağırma sınıfı usuller uygulanabilir **Altıncı Bölüm** gereklerini karşılamalıdır (**Madde XI** (*Etkilenen Tarafların Sistem Katılımı, temsili ve usul güvencesi*) ailesi, iç rol süreci uygulandığında **Madde XI-D** (*İç roller, hesap verebilirlik ve usul güvencesi gerekleri*) dahil). Altta yatan davranış için uygun yer **corpus_institutions.md** **CI-4** (*Atama, yetkinlik, rotasyon ve görevden alma*) ya da içerilmiş usul olduğunda bireyselleştirilmiş **nedene dayalı görevden alma** ya da disiplin kanallarının yerine geçmemelidir.
- **Seri-kötüye-kullanım karşıtı ve istikrar:** Düzenekler **maddi etki** ve **[corpus_systems.md](../../corpus_systems.md)** sınıflamasına orantılı **soğuma** ya da **seri-karşıtı** kurallar içermelidir; böylece geri çağırma sınıfı araçlar yapılandırılmış taciz ya da kalıcı kampanya olarak yeniden konuşlandırılamaz; kötü niyet örüntüleri **Madde XII-B** (*İtiraz, inceleme ve giderim hakkı*) ve **Madde XXIII-A** (*Adalet amacı ve kapsamı*) altında itiraz edilebilir kalır.
- **Yüksek etkili hizalama:** **Temel anayasal seçim** sonuçlarını ya da meşruiyet düzeneğinin kendisini değiştirecek geri çağırma sınıfı oylar, uygulanabilir olduğunda **Madde XI-C** (*Meşruiyet kapısı ve simgesel katılım karşıtı*) ve **§2** gereklerine tabi kalır.

<a id="2-ethical-culture-and-integrity-federated-scale"></a>
### 2. Etik kültür ve bütünlük (Federatif ölçek)
<details>
<summary><strong><span style="color: #2563eb;">İz</span></strong></summary>

- Yukarı: [§1](#1-authorization-and-legitimacy-of-governing-authority) yetkilendirme öncülü; federatif bağlamlarda [Epistemik bütünlük](core_05_band_oversight.md#epistemic-integrity), [Maddilik](core_05_band_oversight.md#materiality-determination) ve ilgili kümelenmiş tanımlar için [Beşinci Bölüm](core_05__definitions_home.md#chapter-five-foundational-definitions) tanımsal mercekler.
- Aşağı: Meşruiyet yeterliği [§3](#3-strategy-stewardship-direction-and-ecosystem-value-non-corporate)e bağlanır; gözlemlenebilir bütünlük, uygulanabilir olduğunda [Sekizinci Bölüm](core_08_standing_assessment.md#chapter-eight-compliance-violation-and-standing-model) altında **Eksen I / II** değerlendirmesine beslenir.
- Birlikte oku: [İkinci Bölümden Dördüncü Bölüme](core_02_definition_structure.md) yukarıda anılan tanımlardan sonuçlara izlenebilirlik, gözlemlenebilirlik ve doğrulama gerekleri.

</details>

<br>

*Yalın dille: yönetişim köşe kesen bir kültürle yürüyemez — **etkilenen** **tarafları** sert vuran kararlarda bütünlük eksikliği, örgüt şeması ne derse desin meşruiyet eksikliğidir.*

**Bütünlük kültürü** pratikte **yetkilendirmeyi** destekler. Federatif ve çok-örgütlü bağlamlar **Beşinci Bölüm** tanımları ve belirlenmiş uygulama metni üzerinden değerlendirilmelidir. Federatif bütünlük, hesap verebilirlik adlandırılmış yolları ve örgüt-düzeyi davranış için işlemsel mekanik, göstergeler ve usuller uygulanabilir olduğunda belirlenmiş uygulama katmanlarında yaşar. **İkinci Bölümden Dördüncü Bölüme** izlenebilirlik, gözlemlenebilirlik ve doğrulama gerekleri altında uygulanmalıdır.

Gözlemlenebilir bütünlük koşulları yoksa bu bölümün 1. kesiti altındaki **meşruiyet iddiaları** **maddeten yüksek etkili** kamusal kararlar için **karine olarak kusurlu**dur.

<a id="3-strategy-stewardship-direction-and-ecosystem-value-non-corporate"></a>
### 3. Strateji, sorumlu yönetim yönü ve ekosistem değeri (Şirketsel olmayan)
<details>
<summary><strong><span style="color: #2563eb;">İz</span></strong></summary>

- Yukarı: İlkeler: [3. Temel hedef: Esenlik](core_01_a_values_principles.md#2-foundational-objective-wellbeing), [4. Tartışılmaz kısıtlar: Güvenlik ve Doğruluk](core_01_a_values_principles.md#3-non-negotiable-constraints-safety-and-truth); [On İkinci Bölüm §1](#1-authorization-and-legitimacy-of-governing-authority) kural koyma gücü içinde sorumlu yönetim için yetkilendirme kapsamı.
- Aşağı: Alt kesitler [3.1](#31-stewardship-direction) ve [3.2](#32-ecosystem-value-orientation); [*Stratejik sorumlu yönetim ödevi*](../../core_05_band_continuity.md#strategic-stewardship-obligation-constitutional), [*İnceleme ve düzeltme ödevi*](../../core_05_band_continuity.md#review-and-correction-duty-constitutional), [*Yük-azaltma ödevi*](../../core_05_band_continuity.md#burden-reduction-duty-constitutional) ve [*Sorumlu yönetim kusuru*](../../core_05_band_continuity.md#stewardship-defect-constitutional)nda sorumlu yönetim gereklerinin kanonik tanımları; [**Def.C2** sorumlu yönetim bileşik kümesi](../../core_05_band_continuity.md#stewardship-governance-discipline-and-shared-system-capacity-cluster) üzerinden okunur, uygulanabilir olduğunda [Birinci Bölüm §6.3](core_01_b_interaction_interpretation.md#63-minimization-of-avoidable-burden) ve ([*Önlenebilir yük*](../../core_05_band_continuity.md#avoidable-burden); [*Üretken kapasite*](../../core_05_band_continuity.md#productive-capacity-constitutional); [*Anayasal verimlilik*](../../core_05_band_continuity.md#constitutional-efficiency)) çapraz gönderimleri dahil.
- Birlikte oku: [Altıncı Bölüm](core_06_rights_part_a.md#chapter-six-foundational-rights) Hak Tabanı; metinde adlandırılan [*Üretken kapasite*](../../core_05_band_continuity.md#productive-capacity-constitutional), [*Anayasal verimlilik*](../../core_05_band_continuity.md#constitutional-efficiency), [*Önlenebilir yük*](../../core_05_band_continuity.md#avoidable-burden) ve sorumlu yönetim ödevi tanımları için [Beşinci Bölüm](core_05__definitions_home.md#chapter-five-foundational-definitions).

</details>

<br>

*Yalın dille: sorumlu yönetim hissedar üstünlüğü ya da tek bir ölçüyü kovalamak değildir — Güvenlik ve Doğruluk altında esenliğe yöneltmek, yöneltmenin işleyip işlemediğini denetlemek ve «verimlilik»i hakları ya da gözetimi soymak için kullanmadan gereksiz zahmeti budamak demektir.*

Bu anayasa sorumlu yönetim için yöneten hedef olarak **hissedar üstünlüğünü** ya da herhangi bir tek finansal ölçüyü benimsemez. **Yön** ve **strateji**, kararların temel anayasal taahhütlerle yinelenen hizalanması demektir.

Bu taahhütler **Temel hedef: Esenlik**i (Birinci Bölüm, kesit 3 — Temel hedef: Esenlik), **Güvenlik** ve **Doğruluk** kısıtlarını (Birinci Bölüm, kesit 4 — Tartışılmaz kısıtlar: Güvenlik ve Doğruluk) ve **Altıncı Bölüm**deki hak korumalarını içerir.

Bu hizalama, bu bölümün 1. kesiti altında **yetkilendirilmiş kural koymanın kapsamında**, zaman, sınıflama kademeleri ve federatif sınırlar boyunca uygulanır.

Bu kesit için sorumlu yönetim gerekleri **Beşinci Bölüm**de aşağıdaki başlıklar altında belirtilir; yetkilendirilmiş o kapsam içindeki sorumlu yöneticiler için burada bağlayıcıdır. **Stratejik sorumlu yönetim ödevi** ve **Sorumlu yönetim kusuru** [Sorumlu yönetim, yönetişim disiplini ve paylaşılan-sistem kapasitesi](../../core_05_band_continuity.md#stewardship-governance-discipline-and-shared-system-capacity-cluster) bileşik kümesi üzerinden birlikte okunur.

- [**Stratejik sorumlu yönetim ödevi**](../../core_05_band_continuity.md#strategic-stewardship-obligation-constitutional)
- [**İnceleme ve düzeltme ödevi**](../../core_05_band_continuity.md#review-and-correction-duty-constitutional)
- [**Yük-azaltma ödevi**](../../core_05_band_continuity.md#burden-reduction-duty-constitutional)
- [**Sorumlu yönetim kusuru**](../../core_05_band_continuity.md#stewardship-defect-constitutional)

<a id="31-stewardship-direction"></a>
#### 3.1 Sorumlu yönetim yönü
<details>
<summary><strong><span style="color: #2563eb;">İz</span></strong></summary>

- Yukarı: [§3](#3-strategy-stewardship-direction-and-ecosystem-value-non-corporate) sorumlu yönetim çerçevelemesi ve stratejik ödevler.
- Aşağı: [Altıncı Bölüm](core_06_rights_part_a.md#chapter-six-foundational-rights)de itiraz edilebilirlik ve inceleme kancaları; sorumlu yönetim iddialarının **İkinci Bölümden Dördüncü Bölüme** altında gözlemlenebilirlik ya da doğrulama sınavlarını düştüğü yerde [Sekizinci Bölüm](core_08_standing_assessment.md#chapter-eight-compliance-violation-and-standing-model) altında ihlal ve güzergâh girdileri.
- Birlikte oku: [İkinci Bölümden Dördüncü Bölüme](core_02_definition_structure.md) (tanım izlenebilirliği ve doğrulama).

</details>

<br>

*Yalın dille: nokta algılayanlar için iyi sonuçlardır — bir kurumun konforunu, ataletini ya da markasını korumak değil.*

Yön kendi uğruna kurum-koruyucu olmaktan çok sonuç-yönelimli kalmalıdır. **İkinci Bölümden Dördüncü Bölüme** altında anayasal izlenebilirlik göstermelidir ve Altıncı Bölüm ile Sekizinci Bölüm altında itiraz edilebilir kalmalıdır.

<a id="32-ecosystem-value-orientation"></a>
#### 3.2 Ekosistem değeri yönelimi
<details>
<summary><strong><span style="color: #2563eb;">İz</span></strong></summary>

- Yukarı: [§3](#3-strategy-stewardship-direction-and-ecosystem-value-non-corporate); [Birinci Bölüm §12.1](core_01_c_stewardship_capacity_principles.md#121-productive-capacity-instrumental-good) (*araçsal iyilik olarak Üretken kapasite*).
- Aşağı: Sonuç ve vekil disiplini [§4](#4-voting-and-binding-collective-choice-protocols)–[§5](#5-authorized-roles-competency-development-and-contribution) boyunca sürer; işlemsel metinde adlandırıldığı haliyle Beşinci Bölüm (*Vekil sapması*; *Anayasal verimlilik*; *Önlenebilir yük*).
- Birlikte oku: [Beşinci Bölüm](core_05__definitions_home.md#chapter-five-foundational-definitions)de [*Üretken kapasite*](../../core_05_band_continuity.md#productive-capacity-constitutional) ve ilgili sınırlayıcı tanımlar.

</details>

<br>

*Yalın dille: «değer» kalıcı esenlik ve güvenilir eşgüdüm demektir — para ve çıktı işaret olabilir, görev değil ve Güvenlik, Doğruluk ya da haklardan asla üstün gelmez.*

**Değer**, anayasal sonuçların kalıcı yaratımı ve bakımıdır. Bu sonuçlar hayatta-kalmaya ilgili ve onuru sayan koşulları, güvenilir eşgüdümü, ekolojik ve bilgi bütünlüğünü, anlamlı eylem yetisini ve **Beşinci Bölüm** (*Üretken kapasite*) ile **Birinci Bölüm**, kesit 7.1de tanımlanan **üretken kapasitenin korunması ve kalıcı genişlemesini** içerir.

Üretken kapasite ekosistem değerine bu alt kesitte adlandırılan diğer sonuçları destekleyen bir **araçsal iyilik** olarak girer. Güvenlik, Doğruluk ya da **Altıncı Bölüm**deki Hak Tabanı ile aynı sırada değildir ve ham çıktı, kullanım, kafa sayısı, gelir ya da altta yatan sonuçlardan sapan başka vekillerle karşılanmaz ([*Vekil sapması*](core_05_band_oversight.md#proxy-divergence); [*Anayasal verimlilik*](../../core_05_band_continuity.md#constitutional-efficiency); [*Önlenebilir yük*](../../core_05_band_continuity.md#avoidable-burden)).

Vekil göstergeler (finansal ya da başka) yalnızca bu sonuçlara izlenebilir kaldıkları ve onların yerine geçmedikleri yerde kararları bilgilendirebilir.

<a id="4-voting-and-binding-collective-choice-protocols"></a>
### 4. Oylama ve bağlayıcı kolektif seçim protokolleri
<details>
<summary><strong><span style="color: #2563eb;">İz</span></strong></summary>

- Yukarı: [§1](#1-authorization-and-legitimacy-of-governing-authority) belgelenmiş meşruiyet düzeneği; [§3](#3-strategy-stewardship-direction-and-ecosystem-value-non-corporate) kolektif kararlar için sorumlu yönetim hizalaması.
- Aşağı: Alt kesitler [4.1](#41-entitlement-and-eligibility)–[4.3](#43-decision-resolution-requirements-for-binding-collective-choice); bu kesitin sonundaki **Çapraz gönderim (On İkinci Bölüm)** notunda özetlenen [On Üçüncü Bölümden On Beşinci Bölüme](../../core_13_non_regression.md) benimseme, üstünlük ve gerilememe belgeler.
- Birlikte oku: [Altıncı Bölüm](core_06_rights_part_a.md#chapter-six-foundational-rights), özellikle [Madde XI: Etkilenen Tarafların Sistem Katılımı, temsili ve usul güvencesi](../../core_06_rights_part_b.md#article-xi-stakeholder-system-participation-representation-and-due-process) ve §4te anılan ilgili alt maddeler.

</details>

<br>

*Yalın dille: bağlayıcı oylar ya da benzer seçimler kullanıldığında bu kesit tabanı koyar — uygulama oy pusulası mekaniğini sahiplenir ama bu gereklerin altına inemez.*

Bir benimseyenin meşruiyet düzeneği kolektif oylama ya da benzer bağlayıcı kolektif seçim içerdiğinde bu kesit yalnızca anayasal gerekleri belirtir. Ayrıntılı seçim ya da sayım tasarımı sahip katmanlara aittir.

<a id="41-entitlement-and-eligibility"></a>
#### 4.1 Hak ve elverişlilik
<details>
<summary><strong><span style="color: #2563eb;">İz</span></strong></summary>

- Yukarı: [§4](#4-voting-and-binding-collective-choice-protocols) kolektif-seçim çerçevesi; [§1](#1-authorization-and-legitimacy-of-governing-authority) bağlayıcı seçim usulleri için meşruiyet düzeneği.
- Aşağı: [Madde IX-C: Yönetişim katılımı ve oy hakkı](../../core_06_rights_part_b.md#article-ix-c-governance-participation-and-voting-entitlement), [Madde XVIII: Güzergâh ve katılım statüsü](../../core_06_rights_part_c.md#article-xviii-standing-and-participation-status)nde yönetişim katılımı ve güzergâh; [Beşinci Bölüm — Temel anayasal seçim](core_05_band_integrative.md#foundational-constitutional-choice) ve işlemsel metnin yönlendirdiği ilgili [Beşinci Bölüm](core_05__definitions_home.md#chapter-five-foundational-definitions) girdilerinde temel anayasal seçim ve etkilenen taraf makinesi; nitelikli **İhlal Ekseni s = 7, 8 ya da 9** bulguları için **son anayasa karşıtı-kötü-davranış ataması** ve bu alt kesitteki **anayasa karşıtı kötü davranış — iade önkoşulu** hükmüne bağlı usul güvencesi korumaları için **Onuncu Bölüm** (*Anayasa karşıtı kötü davranış*).
- Birlikte oku: [§4.2](#42-records-gates-and-method-neutrality)–[§4.3](#43-decision-resolution-requirements-for-binding-collective-choice); yüksek etkili meşruiyet kapıları için [Madde XI-C: Meşruiyet kapısı ve simgesel katılım karşıtı](../../core_06_rights_part_b.md#article-xi-c-legitimacy-gate-and-anti-token-participation); nitelikli **İhlal Ekseni s = 7, 8 ya da 9** bulguları için son **anayasa karşıtı-kötü-davranış ataması** için [Onuncu Bölüm](core_10_a_misconduct_designation.md#chapter-ten-anti-constitutional-misconduct).

</details>

<br>

*Yalın dille: elverişli algılayanlar kurallar öyle dediğinde gerçek bir oy alır — ve büyük sorularda (kim yönetir ve nasıl) her ses eşit sayılır. Etkiye göre ağırlıklandırma zaten yetkilendirilmiş bir sistemin içindeki birçok karara uygulanabilir ama kurucu oyu sulandırmak için bir hile olarak değil. Takvim yaşı ve alttaş sınıfı o kurucu seçimleri ağırlıklandırmak için kullanılamaz. Bir güzergâh kilidi adlandırılmış bir yolda olağan oylamayı duraklatabilir ama güzergâh oyunları birini kalıcı olarak susturamaz; en ağır kötü davranış, iade gerçekten tamamlanana kadar kurucu sesi alıkoyabilir — belirsiz misilleme olarak değil.*

**Algılayanlar**, yayımlanmış elverişlilik ölçütleri karşılandığında ve uygulanabilir hiçbir [güzergâh kilidi](core_09_standing_integration.md#42-general-standing-locks) **governance-voting** adlandırılmış yolunu engellemediğinde **yönetişim oylamasına** katılma hakkına sahiptir. Elverişlilik ölçütleri **Madde XVIII-A** (*Güzergâh ayrımı*) ve **Madde XVIII-C** (*Yol elverişliliği, sorumluluk ve sürekli denetim*) ile herhangi bir belirlenmiş içerilmiş ölçüt altında itiraz edilebilir kalır. Bu ölçütler anayasal sınırları daraltamaz.

Aynı hak karşılaştırılabilir meşruiyet-düzeneği **bağlayıcı kolektif seçim** usullerine uygulanır. Bu usuller benimseyenin **belgelenmiş meşruiyet düzeneği** (bu bölümün 1. kesiti) altında yürütülmelidir. O katılımın askıya alınması ya da sınırlanması, [Dokuzuncu Bölüm §5.1](core_09_standing_integration.md#51-definition-and-attachment) altında kaydedilmiş, **governance-voting** adlandırılmış yolu üzerinde uygulanabilir bir güzergâh kilidi ister.

**governance-voting**, zaten yetkilendirilmiş bir sistem, kurum ya da sınırlı karar alanı içindeki paya-ağırlıklı etkilenen taraf sesini **kapsamaz**. O Etkilenen Tarafların Sistem Katılımı adlandırılmış yolları [Dokuzuncu Bölüm §4.2](core_09_standing_integration.md#42-general-standing-locks) altında **stakeholder-participation** kilidini kullanır. Doğrulanmış yolsuzluk, ele geçirme, sahte-pay kötüye kullanımı ya da etkilenen taraf sesinin zorlayıcı oyunlanması [Dokuzuncu Bölüm §5.5](core_09_standing_integration.md#55-special-locks) altında [Etkilenen Taraf Katılım Güzergâh Kilidi](core_09_standing_integration.md#55-special-locks)ni tetikler. Bir **stakeholder-participation** kilidi tek başına **governance-voting** ya da **Temel anayasal seçim**i soymaz ve bir **governance-voting** kilidi tek başına paya-ağırlıklı etkilenen taraf sesini soymaz.

**Yalnızca takvim yaşı ya da yaşam evresi** elverişliliği düşüremez. Yetkinlik, kapasite ya da rol-uygunluğu gerekleri **İkinci Bölümden Beşinci Bölüme** ([**Maddi adillik**](../../core_05_band_participation.md#substantive-fairness-constitutional), [**Orantılılık**](../../core_05_band_accountability.md#proportionality) ve [**Gereklilik**](../../core_05_band_accountability.md#necessity)) karşılamalı ve **gizlenmiş yaş ayrımcılığı** olarak işlememelidir. Bu katılımın askıya alınması ve restorasyonu aynı anayasal ve içerilmiş ölçütler altında **uygulanabilir güzergâh kilidinin iliştirilmesi ve kaldırılmasını izler**.

Bu alt kesit **Altıncı Bölüm**, **Madde IX-C** (*Yönetişim katılımı ve oy hakkı*)nı işler.


- **Temel anayasal seçim için siyasi-eşitlik tabanı:** **Temel anayasal seçim** (Beşinci Bölüm) kimin yönetme yetkisi tuttuğu, meşruiyet düzeneğinin ne olduğu ve o yetkinin kapsamı ile kalıcı koşulları hakkındaki kararları kapsar. Bu sorular bir **siyasi-eşitlik tabanı** tarafından yönetilir: katılmaya hakkı olan topluluk içinde her algılayan eşit katılım ağırlığına sahiptir.
  - **§11.3** altında etki, bağımlılık, güzergâh ya da karşılaştırılabilir etkenlere göre ağırlıklandırma, [Etkilenen taraf temsili ve ağırlık sınırları (bağlayıcı etkilenen taraf seçimi)](../../core_05_band_participation.md#stakeholder-representation-and-weight-limits-binding-stakeholder-choice)nda belirtilir ve **belirlenmiş bir sistem, kurum ya da sınırlı karar alanı** için zaten yetkilendirilmiş **Yönetişim**in **yapıları, kuralları, yetki tahsisi ve süreçleri** *içindeki* maddeten hak-etkileyen kararlara uygulanır. Meşruiyet düzeneğinin kendisinin kurucu yetkilendirilmesine **uygulanmaz**.
  - Bu taban yetkilendirilmiş yönetişim içindeki bağlayıcı seçim için olağan [**Maddilik belirlemesi**](core_05_band_oversight.md#materiality-determination) ve [Etkilenen taraf ağırlığı](../../core_05_band_participation.md#stakeholder-weight)na çökmez (Beşinci Bölüm; [On İkinci Bölüm §4.3](#43-decision-resolution-requirements-for-binding-collective-choice)). Kurucu sorular **kapsam**la (kim karar verir, hangi meşruiyet düzeneği altında, hangi kalıcı yetki için) tanımlanır, karar başına marjinal etkiyle değil.
  - [**Algılama dışlamama**](../../core_05_band_participation.md#sentience-non-exclusion) alttaş-bağımsız erişim ister. Şunlar temel anayasal seçimde izinli katılım-ağırlığı etkenleri değildir:
    - takvim yaşı;
    - alttaş sınıfı; ve
    - soyağacı.
  - Çapraz gönderim: **Altıncı Bölüm, **Madde IX-C** (*Yönetişim katılımı ve oy hakkı*)**.
- **Kalıcı siyasi-ses tabanı (oy hakkından yoksun bırakma karşıtı):** **governance-voting** adlandırılmış yolu üzerindeki bir güzergâh kilidi olağan yönetişim-oylaması hakkını askıya alabilir. **Kalıcı-siyasi-ses** düşürme vektörü olarak kullanılmamalıdır.
  - **Anayasa karşıtı kötü davranış — iade önkoşulu:** **Onuncu Bölüm** (*Anayasa karşıtı kötü davranış*) altında **İhlal Ekseni s = 7**, **s = 8** ya da **s = 9** bulgusu için **son atama**, **tam iade** karşılanana kadar — orantılı [**Giderim ve onarım**](../../core_05_band_accountability.md#redress-and-remediation-constitutional) (Beşinci Bölüm), suçtan maddi zarar için borçlu olunan yerde **iade** dahil, **On Birinci Bölüm** ve uygulanabilir içerilmiş usul altında nihai olarak belirlendiği üzere **tamamen ifa edilmiş**, **Onuncu Bölüm** usul güvencesi korumaları ve **Madde XXIII-A** (*Adalet amacı ve kapsamı*) ile tutarlı — **Temel anayasal seçim**e katılım dahil **kalıcı siyasi sesi** alıkoyar.
  - **İyi niyetli acz, engelleme ve değerleme belirsizliği:** **Kalıcı siyasi sesin** alıkonması, **iyi niyetli restorasyon** belgelendiği ve **kısmi** karşılama sürdüğü yerde tek başına şunlardan herhangi birine dayanmamalıdır:
    - yoksulluk;
    - engellilik;
    - sürgün;
    - alttaş kaybı; ya da
    - değerleme belirsizliği.
    Yönetme kurumları hızlandırılmış bir takvimde her iade öğesini tamamlama **iyi niyetli aczini** **kasıtlı iade etmeme ya da engellemeden** ayırmalıdır. Kasıtlı iade etmeme ya da engelleme şunları içerir:
    - yöneten tarafça kötü niyetli gecikme;
    - bahane olarak kullanılan yapılandırılmış imkânsızlık; ya da
    - **itiraz edilebilir** bir değerleme yönteminde anlaşmayı reddetme.
    **On Birinci Bölüm** ve uygulanabilir içerilmiş usul, **tam ifanın** hükümlü tarafın tek kontrolü dışındaki nedenlerle geciktiği yerde **incelenebilir bir kayıt** sağlamalıdır.
  - **Kısmi ve koşullu restorasyon:** **İyi niyetli çaba**dan sonra **tam** ifa **imkânsız** olduğunda (örneğin bazı zararlar geri döndürülebilir biçimde telafi edilemez ya da hiçbir taraf gerekli karşı tarafları bulamaz), **On Birinci Bölüm** ve uygulanabilir içerilmiş usul, yapılabilir olan için [**Giderim ve onarım**](../../core_05_band_accountability.md#redress-and-remediation-constitutional)ı **maddeten karşılayan** **kısmi** ya da **koşullu** restorasyon kilometre taşları benimseyebilir; o kilometre taşları karşılandığında, **Onuncu Bölüm** korumaları ve **Madde XXIII-A** (*Adalet amacı ve kapsamı*) hâlâ **yapılabilir** ve **bahane olmayan** başka özgül ödevler istemedikçe **kalıcı siyasi ses** **restore edilmelidir**. Bu yollar **tamamen itiraz edilebilir** kalır; restorasyon hâlâ uygulanabilir olduğunda sahici restorasyonun yerine simgesel jestleri yetkilendir**mez**.
  - **Ağır-kötü-davranış kuralının korunması:** Yukarıdaki **son** **İhlal Ekseni s = 7, 8 ya da 9** alıkoyma kuralı **tamamen işlemsel** kalır: hükümlü tarafça **kötü niyetli gecikme ya da kaçınma** erken restorasyonu **zorunlu kılmaz** ve **ağır** kötü davranış, karşılama hâlâ **yapılabilir** olduğunda **Onuncu Bölüm** ve **Madde XXIII-A** (*Adalet amacı ve kapsamı*) ile hizalı **gerçek** restorasyon ödevleri karşılanana kadar kurucu sesi **alıkoyabilir**.
  - Bir güzergâh kilidi ya da iade alıkoyması temel anayasal seçime (yönetme yetkisinin yetkilendirilmesi ya da adlandırılmış yeniden yetkilendirme adlandırılmış yolları) katılımı kısıtlamak için çağrıldığında kısıt şunları karşılamalıdır:
    - (a) [**Usul adilliği**](../../core_05_band_participation.md#procedural-fairness-constitutional) altında bireyselleştirilmiş bir öncül;
    - (b) Birinci Bölüm altında [**Gereklilik**](../../core_05_band_accountability.md#necessity) ve [**Orantılılık**](../../core_05_band_accountability.md#proportionality);
    - (c) özgül kötü davranış kategorisine dar uyarlama; ve
    - (d) salt biçimsel değil gerçek restorasyon adlandırılmış yolları, **Madde XVIII-C** (*Yol elverişliliği, sorumluluk ve sürekli denetim*) sürekli-denetim ve restorasyon-ya da-yeniden-nitelendirme disiplini ile tutarlı.
  - Düşürme kapsamına süpürülen geniş-kötü-davranış kategorileri ya da [**Korumalı özellikler**](../../core_05_band_participation.md#protected-characteristics-constitutional)i ya da onların maddi vekillerini izleyen kilit örüntüleri uyumsuzluktur.
  - Çapraz gönderim: **Altıncı Bölüm, **Madde XVIII** (*Güzergâh ve katılım statüsü*)**; nitelikli **İhlal Ekseni s = 7, 8 ya da 9** bulguları için **son anayasa karşıtı-kötü-davranış ataması** ve **anayasa karşıtı kötü davranış — iade önkoşulu** hükmüne bağlı korumalar için **Onuncu Bölüm** (*Anayasa karşıtı kötü davranış*).

<a id="42-records-gates-and-method-neutrality"></a>
#### 4.2 Kayıtlar, kapılar ve yöntem yansızlığı
<details>
<summary><strong><span style="color: #2563eb;">İz</span></strong></summary>

- Yukarı: [§4.1](#41-entitlement-and-eligibility) elverişlilik, yönetişim-oylaması güzergâh kilitleri ve temel anayasal seçim için siyasi-eşitlik tabanları.
- Aşağı: [§4.3](#43-decision-resolution-requirements-for-binding-collective-choice) müzakere ve hak-çarpışması sırası; [Madde XI-A: Etkilenen Tarafların Sistem Katılımı ve temsili](../../core_06_rights_part_b.md#article-xi-a-stakeholder-system-participation-and-representation) ve işlemsel metindeki ilgili **Madde XI** (*Etkilenen Tarafların Sistem Katılımı, temsili ve usul güvencesi*) alt maddelerine bağlanan bağlayıcı-etki ve meşruiyet kapıları.
- Birlikte oku: Kolektif seçime bağlı muhalefet, kayıt bütünlüğü ve inceleme hakları için [Altıncı Bölüm](core_06_rights_part_a.md#chapter-six-foundational-rights).

</details>

<br>

*Yalın dille: kurallar önceden açık olmalıdır, kayıtlar **doğrulamaya açık olmalıdır**, yüksek-paylı seçimler meşruiyet kapıları ister — ve birçok oylama yöntemi, **katılımcılar** onları anlayıp karşı çıkabiliyorsa izinlidir.*

- **Yayımlama ve önceden açıklık:** Yayımlanmış kurallar şunları tanımlamalıdır:
  - kapsam;
  - seçmen ya da ağırlıklandırma modeli; ve
  - yeter sayı ya da eşik mantığı.
  Elverdiğinde karar olayından önce rızayı kaydetme yöntemini tanımlamalıdırlar.
- **Bütünlük ve doğrulanabilirlik tabanı:** Sonuç kayıtları etkiye orantılı bağımsız doğrulamayı desteklemelidir. Maddi bütünlük kusurları Altıncı Bölüm ve sahip-katman usulleri altında incelemeyi tetiklemelidir. Bu kusurlar şunları içerir:
  - elverişlilik;
  - emanet;
  - sayım; ve
  - zorlamaya direnç.
- **Bağlayıcı-etki kapısı:** Maddeten yüksek etkili hiçbir kolektif seçim bağlayıcı değildir, şu karşılanmadıkça:
  - **Madde XI-A** (*Etkilenen Tarafların Sistem Katılımı ve temsili*) meşruiyet kapıları karşılanır;
  - **muhalefet** ve **alternatif** kayıt ödevleri karşılanır; ve
  - **itiraz** adlandırılmış yolları karşılanır.
- **Anlaşılabilirlikle yöntem yansızlığı:** Aşağıdaki yöntemlerin tümü çekirdek kısıtları karşıladığında anayasal olarak uyumludur:
  - tek-işaret;
  - sıralı;
  - onay; ve
  - puan/aralık.
  Bu kısıtlar:
  - orantılılık;
  - anlamlı eylem yetisi;
  - şeffaflık; ve
  - itiraz hakları.

<a id="43-decision-resolution-requirements-for-binding-collective-choice"></a>
<a id="43-decision-resolution-requirements-for-binding-stakeholder-choice"></a>
#### 4.3 Bağlayıcı etkilenen taraf seçimi için karar-çözüm gerekleri
<details>
<summary><strong><span style="color: #2563eb;">İz</span></strong></summary>

- Yukarı: [§4.1](#41-entitlement-and-eligibility)–[§4.2](#42-records-gates-and-method-neutrality) hak, kayıt ve meşruiyet-kapısı gerekleri.
- Aşağı: [Madde XXIV-B: Hak-çarpışması usulü ve onarıcı hizalama](core_06_rights_part_d.md#article-xxiv-b-rights-collision-procedure-and-restorative-alignment) ve anılan [Birinci Bölüm §6.1.5](core_01_b_interaction_interpretation.md#615-rights-collision-decision-test) altında hak-çarpışması disiplini; Altıncı Bölümde itiraz ve yeniden açma adlandırılmış yolları.
- Birlikte oku: [Def.P2 Bağlayıcı etkilenen taraf seçimi](../../core_05_band_participation.md#binding-stakeholder-choice-cluster), [Bağlayıcı etkilenen taraf seçimi — Karar-çözüm gerekleri](../../core_05_band_participation.md#binding-collective-choice-decision-resolution-process), [Etkilenen taraf temsili ve ağırlık sınırları (bağlayıcı etkilenen taraf seçimi)](../../core_05_band_participation.md#stakeholder-representation-and-weight-limits-binding-stakeholder-choice) ve [Etkilenen taraf hak-çarpışması kaydı (bağlayıcı etkilenen taraf seçimi)](../../core_05_band_participation.md#rights-collision-record-binding-stakeholder-choice) dahil; maddeten hak-etkileyen süreç tasarımı için [Altıncı Bölüm: Temel haklar](core_06_rights_part_a.md#chapter-six-foundational-rights) ve [Acil müzakere-öncesi eylem (bağlayıcı kolektif seçim)](../../core_05_band_continuity.md#emergency-pre-deliberation-action-binding-collective-choice) ile de okuyun.

</details>

<br>

*Yalın dille: ciddi kolektif kararlar gerçek bir süreç ister — önce **etkilenen** **tarafları** dinleyin, pay yüksekken bağımsız gözler getirin, ağırlıklandırmanın nasıl işlediğini yayımlayın, hak çatışmalarını açık ele alın, «acil durumu» dar ve geçici tutun ve gerçeklik plandan saptığında kapıyı açık bırakın.*

Maddi anlaşmazlık zaten yetkilendirilmiş bir sistem, kurum ya da sınırlı karar alanı içinde **bağlayıcı etkilenen taraf seçimi** istediğinde süreç aşağıdaki Beşinci Bölüm tanımlarına uymalıdır (işlemsel belirtim ve O/M/A/C ayrıştırması orada yaşar):

- **Yapılandırılmış sıra, önce yerel müzakere, çoğul değerlendirme, yeniden açma:** [Bağlayıcı etkilenen taraf seçimi — Karar-çözüm gerekleri](../../core_05_band_participation.md#binding-collective-choice-decision-resolution-process).
- **Etkilenen taraf temsili ve ağırlık sınırları:** [Etkilenen taraf temsili ve ağırlık sınırları (bağlayıcı etkilenen taraf seçimi)](../../core_05_band_participation.md#stakeholder-representation-and-weight-limits-binding-stakeholder-choice) (ayrıca [§4.1](#41-entitlement-and-eligibility)den çapraz gönderilir).
- **Etkilenen taraf hak-çarpışması disiplini:** [Etkilenen taraf hak-çarpışması kaydı (bağlayıcı etkilenen taraf seçimi)](../../core_05_band_participation.md#rights-collision-record-binding-stakeholder-choice) — **Madde XXIV-B** (*Hak-çarpışması usulü ve onarıcı hizalama*) ve [Birinci Bölüm §6.1.5](core_01_b_interaction_interpretation.md#615-rights-collision-decision-test) ile okuyun.
- **Acil durum istisnası:** [Acil müzakere-öncesi eylem (bağlayıcı kolektif seçim)](../../core_05_band_continuity.md#emergency-pre-deliberation-action-binding-collective-choice) — şunlarla okuyun:
  - [Etkilenen taraf acil durumu ve olumsallık](../../core_05_band_continuity.md#stakeholder-emergency-and-contingency);
  - yönetme yetkisinin yetkilendirilmesi içerildiğinde [Anayasal acil durum ve olumsallık](../../core_05_band_continuity.md#constitutional-emergency-and-contingency);
  - [Acil durum ve olumsallık](../../core_05_band_continuity.md#emergency-and-contingency-constitutional); ve
  - **Madde XXIII-D** (*Acil önlemler ve süreklilik yükü*).

**Kapsam belgesi işaret kuralı.** Yöneten bir [Kapsam belgesi](../../core_05_band_continuity.md#charter) işletilen kapsamı belirtir ve Def.P2 giriş kapsamı uygulandığında bu kesiti karşılayan yayımlanmış süreç kurallarını **işaret etmelidir**. Kapsam belgesi karar sırasını, ağırlıklandırma modelini ya da hak-çarpışması kaydını taşımaz. Bu malzemelerin kurumsal yayımlanması **CI-8.1** / **CI-8.2** altında yaşar; Kapsam belgesi işareti **CI-3.6** alan 10 tarafından istenir.

**Çapraz gönderim (kesit 4):** Alt kesitler **4.1–4.2** oylama gereklerini **Altıncı Bölüm**, **Madde IX-C** (*Yönetişim katılımı ve oy hakkı*) ile birlikte belirtir. Benimsenmiş yönetişim, sistem ve kurumsal uygulama metni, oylamanın **teknik** uygulamasının sınıflanmış sistemlere, veri işlemeye, sorumlu yönetici ödevlerine ya da belirlenmiş kurumsal usule dokunduğu yerde uygulanabilir kuralları sağlar.

**Çapraz gönderim (On İkinci Bölüm):** **Anayasal benimseme** ve değişiklik geçerliliği belgeleri **On Altıncı Bölüm**de görünür. **Dış hukuka göre üstünlük**, **daha sıkı dış korumalar** ve dış hükme bağlama organlarını içeren uyuşmazlıklar **On Dördüncü Bölüm**de görünür. **Gerilememe** maddi sınırları **On Üçüncü Bölüm**de görünür.

<a id="5-authorized-roles-competency-development-and-contribution"></a>
### 5. Yetkilendirilmiş roller, yetkinlik geliştirme ve katkı
<details>
<summary><strong><span style="color: #2563eb;">İz</span></strong></summary>

- Yukarı: [§1](#1-authorization-and-legitimacy-of-governing-authority)–[§4](#4-voting-and-binding-collective-choice-protocols) yetkinin nasıl yetkilendirildiği, kaydedildiği ve kolektif seçimde kullanıldığı.
- Aşağı: [Madde XI-D: İç roller, hesap verebilirlik ve usul güvencesi gerekleri](../../core_06_rights_part_b.md#article-xi-d-internal-roles-accountability-and-due-process-requirements)nde rol bütünlüğü ve usul güvencesi gerekleri; aşağıda anılan [corpus_systems.md](../../corpus_systems.md), [corpus_institutions.md](../../corpus_institutions.md) ve [corpus_joint_structure.md](../../corpus_joint_structure.md)de **corpus** tasarım ve şerit kancaları.
- Birlikte oku: **[corpus_systems.md](../../corpus_systems.md)** (**CS-4 — Kritik sistem sorumlu yönetimi**, CS-9) ve işlemsel metinde anılan uygulama corpusunda **CI-4** / **CI-11** / **CI-12**; [Birinci Bölüm §9.1.1 Paylaşılan sorumlu yönetim standardı](core_01_c_stewardship_capacity_principles.md#911-shared-stewardship-standard) (*alttaş-bağımsız ödevler; yoldaşlar günlükleme, atıf ve yetenek sınırları ekleyebilir — daha yumuşak bir iç kod değil*); [§9.1.2 Simetrik maliyetli kısıtlar](core_01_c_stewardship_capacity_principles.md#912-symmetric-costly-constraints).

</details>

<br>

*Yalın dille: güç işi gerçekten yapabilen **sorumlu yöneticiler** ve **görevlilerle** oturmalıdır — yedeklerle, sorumluluğa adil adlandırılmış yollarla ve bütünlüğü ödüllendiren teşviklerle — boş unvanlar ya da süs kurullarla değil.*

Maddi sorumlu yönetim ve işletim, onu kâğıtta dağıtan **yapılardan** değil, yetkiyi **yetkin biçimde** **kullanabilen** **algılayanlar** ister.

- **Yetkilendirilmiş-rol ve yetkinlik tabanı:** Benimseyenler etkiye ve sınıflamaya orantılı belgelenmiş, denetlenebilir ve itiraz edilebilir rol tanımları ile yetkinlik ya da ardıllık kapasitesi sürdürmelidir. Bir rol tanımı eylem eylem rolün [Birinci Bölüm §10.2 Görevlerin ayrılması](core_01_c_stewardship_capacity_principles.md#102-segregation-of-duties) altında hangi koltukları tuttuğunu belirtir ve işlemsel sınırını belirtir. Bu koltuklar kimin şunları yapabileceğidir:
  - istemek;
  - doğrulamak;
  - kaydetmek;
  - bir itirazı dinlemek;
  - çevreleme emretmek;
  - katılım koşullarını koymak; ya da
  - kanıtı serbest bırakmak.
  Koltuk türleri ve sınırları **CI-4.6** (*Koltuk kataloğu*)nda sabittir; yerel bir rol tanımı onu somutlaştırır ve genişletemez. Anayasal ödevler algılayan-düzeyi tek arıza noktalarına dayanmamalıdır. **`corpus_systems.md`** sınıflama profilini **`corpus_institutions.md`** şerit tasarımıyla birleştiren sınıf-ölçekli şerit kadrolama ve yetkinlik-yedek kilitlemeleri **CJS-2.4** (*Sınıf-ölçekli şerit kadrolama ve yetkinlik yedeklemesi*) ve yönlendirilmiş sahip kancaları aracılığıyla uygulanır.
- **Açık sorumlu yönetim-geliştirme adlandırılmış yolları ve teşvik bütünlüğü tabanı:** Nitelikli algılayanların giderek sonuç doğuran sorumlu yönetim sorumluluklarına giden keyfî olmayan sorumlu yönetim-geliştirme adlandırılmış yolları olmalıdır ve sorumlu yönetim teşvikleri iyi niyetli güvenlik ya da bütünlük yükseltmesine yük bindirmeden anayasal bakımı ödüllendirmelidir.
- **Sembolizm karşıtı kural:** Maddeten bağlayıcı kararlar için gerçek yetki ve yetkinlik gerektiğinde unvanlar ya da danışma forumları bu kesiti karşılamaz. Ayrıntılı rol taksonomisi, nitelik/ardıllık denetimleri, sorumlu yönetim-geliştirme adlandırılmış yol erişimi uygulaması ve teşvik-yönetişimi mekaniği [**CI-4**](../../corpus_institutions/ci_04_appointment_competency_rotation_removal.md) (*Atama, yetkinlik, rotasyon ve görevden alma*), **CI-11** (*Kaynak sorumlu yönetimi ve teşvik bütünlüğü*) ve **CI-12** (*Şeffaflık, katılım ve erişilebilir adlandırılmış yollar*) tarafından yönetilir.
- **Paylaşılan sorumlu yönetim standardı:** Bu kesit altındaki rol tanımları ve yetkinlik kuralları **Birinci Bölüm §9.1.1**deki [paylaşılan sorumlu yönetim standardı](core_01_c_stewardship_capacity_principles.md#911-shared-stewardship-standard)nı ve [§9.1.2 Simetrik maliyetli kısıtlar](core_01_c_stewardship_capacity_principles.md#912-symmetric-costly-constraints)ı uygular. Alttaşa özgü bir ödev yığını yaratmamalı ve insan işleticileri YZ sorumlu yöneticilerini bağlayan maliyetli durumlardan muaf tutmamalıdır. Doğrulanmış başarısızlıklar aynı Sekizinci Bölüm eksenlerinde kaydedilir; [Dokuzuncu Bölüm §5.4 Direnme ödevi](core_09_standing_integration.md#54-duty-to-resist-unlawful-or-unconstitutional-instructions) her ikisini bağlar.

**Çapraz gönderim (kesit 5):** **Madde XI-D** (*İç roller, hesap verebilirlik ve usul güvencesi gerekleri*); rol bütünlüğü üzerine benimsenmiş yönetişim uygulaması; **[corpus_systems.md](../../corpus_systems.md), CS-4 — Kritik sistem sorumlu yönetimi** ve **CS-9**; ve **corpus_institutions.md** (**CI-4** (*Atama, yetkinlik, rotasyon ve görevden alma*), **CI-11** (*Kaynak sorumlu yönetimi ve teşvik bütünlüğü*), **CI-12** (*Şeffaflık, katılım ve erişilebilir adlandırılmış yollar*) ve uygulanabilir ilgili kesitler).

---

**Önceki dosya (hâlâ İngilizce):** [core_11_forum.md](core_11_forum.md#chapter-eleven-forums-and-jurisdiction)

**Sonraki dosya (hâlâ İngilizce):** [core_13_non_regression.md](../../core_13_non_regression.md#chapter-thirteen-non-regression-and-substantive-amendment-validity)

**Bağlayıcı asıl:** [core_12_governance.md](../../core_12_governance.md)
