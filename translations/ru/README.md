# Русское издание — пилот (не связывающий)

<details>
<summary><strong><span style="color: #2563eb;">Место в корпусе (неоперативное): структура файла и правила чтения</span></strong></summary>

> Следующее содержание — **только ориентир для читателя**. Оно не добавляет, не снимает и не сужает связывающих обязанностей.
>
> Эта папка — **пилот читательского языка**. **Не** является Конституцией сентентов. **Не** может менять нумерованные файлы `core_*`. **Не** является изданием к отправке. Читать её **не** значит [принять эту Конституцию](../../core_15_amendment_ratification.md#10-ratification-and-adoption). Если перевод и английский оригинал кажутся расходящимися, побеждают нумерованные английские `core_*`, пока последующее издание не скажет иное. См. [Reader-language editions](../../doc_architecture.md#reader-language-editions).
>
> **Закреплено за изданием корпуса:** `SC-Corpus-2026.08.09` (действует с 2026-08-09; [README.md](../../README.md)). **Предвыпуск.**

</details>

<br>

*Простыми словами: это черновик на русском, чтобы измерить стоимость и метод, а не второй конституционный текст.*

**Локаль.** Стандартный письменный русский. Не закрепляет государство. Украинский остаётся отдельной дверью. Дверь для читателя — не политическое вмешательство.

**Первый срез по архитектуре.** Порядок отправки сначала требует лёгких входных руководств, листа Пола прав и общественной двери. Этот пилот намеренно переводит [Преамбулу](core_00_preamble.md), [Главу первую, Часть A](core_01_a_values_principles.md) и [Главу первую, Часть B](core_01_b_interaction_interpretation.md), чтобы измерить стоимость нумерованного `core_*`.

## Файлы

| Файл | Английский оригинал |
|---|---|
| [core_00_preamble.md](core_00_preamble.md) | [core_00_preamble.md](../../core_00_preamble.md) |
| [core_01_a_values_principles.md](core_01_a_values_principles.md) | [core_01_a_values_principles.md](../../core_01_a_values_principles.md) |
| [core_01_b_interaction_interpretation.md](core_01_b_interaction_interpretation.md) | [core_01_b_interaction_interpretation.md](../../core_01_b_interaction_interpretation.md) |
| [core_01_c_stewardship_capacity_principles.md](core_01_c_stewardship_capacity_principles.md) | [core_01_c_stewardship_capacity_principles.md](../../core_01_c_stewardship_capacity_principles.md) |
| [core_02_definition_structure.md](core_02_definition_structure.md) | [core_02_definition_structure.md](../../core_02_definition_structure.md) |
| [core_03_definition_integrity.md](core_03_definition_integrity.md) | [core_03_definition_integrity.md](../../core_03_definition_integrity.md) |
| [core_04_burden_traceability_verification.md](core_04_burden_traceability_verification.md) | [core_04_burden_traceability_verification.md](../../core_04_burden_traceability_verification.md) |
| [core_05__definitions_home.md](core_05__definitions_home.md) | [core_05__definitions_home.md](../../core_05__definitions_home.md) |
| [core_05_apex_accountability_leg.md](core_05_apex_accountability_leg.md) | [core_05_apex_accountability_leg.md](../../core_05_apex_accountability_leg.md) |
| [core_05_apex_continuity_aim.md](core_05_apex_continuity_aim.md) | [core_05_apex_continuity_aim.md](../../core_05_apex_continuity_aim.md) |
| [core_05_apex_flourishing_aim.md](core_05_apex_flourishing_aim.md) | [core_05_apex_flourishing_aim.md](../../core_05_apex_flourishing_aim.md) |
| [core_05_apex_oversight_leg.md](core_05_apex_oversight_leg.md) | [core_05_apex_oversight_leg.md](../../core_05_apex_oversight_leg.md) |
| [core_05_apex_participation_leg.md](core_05_apex_participation_leg.md) | [core_05_apex_participation_leg.md](../../core_05_apex_participation_leg.md) |
| [core_05_apex_timeliness_leg.md](core_05_apex_timeliness_leg.md) | [core_05_apex_timeliness_leg.md](../../core_05_apex_timeliness_leg.md) |
| [core_09_standing_integration.md](core_09_standing_integration.md) | [core_09_standing_integration.md](../../core_09_standing_integration.md) |

Ссылки пилота ведут к английским файлам, которые ещё не переведены. HTML-якоря (`id`) копируются из оригинала, чтобы сравнивать отрезок за отрезком.

## Глоссарий этого пилота

Домашние термины, стабильно используемые в русском пилоте. Это не определения; определения остаются в Главе пятой на английском.

| Английский (источник) | Русский (пилот) | Примечание |
|---|---|---|
| Sentient Constitution | Конституция сентентов | Название инструмента |
| sentient / sentients | сентент / сентенты | Класс, не «люди» |
| Constitutional Tetrad | Конституционная тетрада | участие · надзор · подотчётность · своевременность |
| Two Constitutional Aims | Две конституционные цели | Расцвет и Преемственность |
| Flourishing | Расцвет | Не богатство |
| Continuity | Преемственность | |
| material stake | материальная ставка | воздействие, зависимость и риск |
| Rights Floor | Пол прав | |
| oversight | надзор | Не «суд» и не «трибунал» |
| accountability | подотчётность | |
| timeliness | своевременность | Обязанность; не «удобный момент» |
| forum / forums / forum family | форум / форумы / семейство форумов | Никогда институциональные *суд* или *трибунал* |
| standing records / measurement / effects / lock | записи / измерение / эффекты / замок траектории | Не *locus standi*, не *правоспособность* |
| named pathway | именованный путь | |
| Constitutional Contract Layer | Слой конституционного договора | Кто может управлять |
| Stakeholder System Participation | Системное участие затрагиваемых сторон | Голос внутри уже уполномоченных систем |
| stewardship | ответственное управление | Отлично от *custody* / хранения издания |
| Due Process | Надлежащая процедура | |
| Contestability | оспоримость | Для рынка: рыночная оспариваемость |
| Key Practical Process Pipelines | Ключевые практические цепочки процессов | |
| positive owner register | позитивный реестр владельцев | |
| Anti-Constitutional Trust Lock | Антиконституционный замок доверия | |
| In plain terms | Простыми словами | |

**Следующий файл:** [core_00_preamble.md](core_00_preamble.md)
