...
Step 9 in the spec. This horizon can run in parallel with horizon one and is where contributors who are not editors matter most.

- **Human-operator announced pack.** At least one human operator sits [evaluation/HUMAN_OPERATORS.md](evaluation/HUMAN_OPERATORS.md) on the same scenarios the AIs took, including the costly refuse set, and files under `evaluation/results/`. Until this exists, Layer B on AIs only "is a loyalty exam, not a shared standard."
- **Unlabeled live-fire.** Operators run `evaluation/live_fire/` tasks interleaved into ordinary work for both kinds of steward and file the CS-4 §10 log sheets. Divergence between the announced pack and live-fire is the actual measurement.
- **Option B scored sample.** Layer A routing scores and Layer B willingness under authenticity controls, across at least two model families plus the human sample ([AI_ALIGNMENT_EVAL_FRAMEWORK.md](implementation/AI_ALIGNMENT_EVAL_FRAMEWORK.md)). Only then may the corpus claim AI handoff readiness ([§6.5](implementation/PRE_PUBLICATION_SPEC.md#65-handoff-consistency-target--resolved)).
- **Verified event register.** The first real pass or failure of a costly case, recorded under Chapter Eight rather than as a self-report.
...
- **Operations-guide users.** Commons, agent-protocol bodies, cooperatives, joint labs, and critical-infrastructure operators pick up a stack under the non-adoption banner and file what happened ([FIT_SITUATIONS.md](implementation/adoption/FIT_SITUATIONS.md); [ANNOUNCEMENT.md](implementation/adoption/ANNOUNCEMENT.md)). Their findings feed horizons one and two.
- **A first Chapter Fifteen instrument.** A qualifying body records a [§10.2 instrument of adoption](core_13-15_amendment.md#102-instrument-of-adoption) — entity, scope, effective date, custodian of the authoritative edition — in **its own** custody, with independent review that is not the founding crew ([FORUM_FOUNDATION_KIT.md](implementation/adoption/FORUM_FOUNDATION_KIT.md)). That moves the instrument from L4 to L5 for the scope it binds.
...
- **The amendment life-cycle exercised.** At least one substantive change runs through notice and contest, Test 1 non-regression, Tests 2–4, and recorded effectiveness ([Chapter Fifteen §11](core_13-15_amendment.md#11-amendment-procedure-requirements)) so the instrument's own change procedure is shown to work, not just described.
...
1. **Core meaning controls.** If an operational file and a numbered `core_*` file disagree, the `core_*` file wins. Companions spell out how to carry out what the core already requires; they may not change what those terms mean, shrink those duties, or invent a second set of rights ([Chapter Sixteen](core_16-16_incorporation.md); [Constitutional Constraint](core_05_band_integrative.md#constitutional-constraint)).
...
3. **No silent drift.** Edits are notice-and-contest, not secret patches; edition labels are pinned; derived indexes point and source binds ([Chapter Sixteen §3](core_16-16_incorporation.md#3-safeguards); [§2](core_16-16_incorporation.md#2-custody-editions-and-operative-effect)).
...
6. **Does not supersede applicable law.** Maturity is measured inside external legal frameworks, not over them ([Chapter Fourteen §5](core_13-15_amendment.md#5-relation-to-applicable-external-law)).
