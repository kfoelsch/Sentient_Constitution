...
- **Verified inputs only** (demonstrable Contribution Axis; [Violation Nature](../core_05_band_accountability.md#violation-nature-chapter-six) for the Violation Axis) per [Verified inputs for standing](../core_09_standing_assessment.md#verified-inputs-for-standing).
- **Constitutional-outcome baseline controls.** Both axes use the unified Chapter Nine thresholds. Numeric estimates must not count raw efficiency, output, prestige, wealth, utilization, burden shifting, allegations, or disfavored status as impact.
- **Recency and currentness** apply only at **Question 3** as gate/readiness inputs under [Chapter Ten §6.1](../core_10_standing_integration.md#61-recency-and-currentness). They must not alter a Chapter Nine contribution standing record, Contribution Axis slot, LEQU measurement, descriptor, or historical recognition. Any stored recency-weighted `C*` value is legacy interoperability data only and must not derive a slot.
- **Unresolved** violation findings are **not** time-discounted for standing integration.
- **Do not** net contribution against violation; **do not** **average** or **substitute** axes. Decide violation remedy, correction, and locks before contribution currentness and competency bars and clearances, per [Chapter Ten §2](../core_10_standing_integration.md#2-automatic-integration-review-and-continuity). An applicable lock controls.
...
**No character multiplier.** Negligence, concealment, coercion, violence, recurrence, and intent remain separately recorded. They may affect safeguards, attribution, remedy, or Chapter Eleven designation, but they do not multiply `x`.
...
**Contribution requirements.** `C_measure` supplies the numeric candidate impact band, while contribution recognition also requires the applicable baseline-state, traceability, non-externalization, and constitutional-alignment findings in core **Chapter Nine §7**. No numeric threshold turns constitutionally defeated output into contribution. Chapter Ten currentness affects only a named gate or readiness decision after violation consequences are decided.

**Default nest** **of** **Chapter Nine** **Contribution Axis** **primary** **bands** (four bands) **into** **slot** **ranges**:
