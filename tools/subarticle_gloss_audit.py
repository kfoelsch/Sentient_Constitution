#!/usr/bin/env python3
"""Require an *In plain terms* line in each Chapter Nine `#### Article …` sub-article.

Operates on `core_09-09_rights_part_*.md`. Stops a sub-article block at the next
...
def subarticle_gloss_errors(root: pathlib.Path) -> list[str]:
    ...
    parts = sorted(root.glob("core_09-09_rights_part_*.md"))
    all_err: list[str] = []
    for path in parts:
...
def main() -> int:
    ...
    parts = sorted(root.glob("core_09-09_rights_part_*.md"))
    if not parts:
        print("subarticle-gloss audit: no core_09-09_rights_part_*.md under --root")
        return 1

    all_err = subarticle_gloss_errors(root)

    print("Subarticle plain-terms audit (Chapter Nine):")
