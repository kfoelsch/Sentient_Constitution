...
Operates on `core_06-06_rights_part_*.md`. Stops a sub-article block at the next
...
def subarticle_gloss_errors(root: pathlib.Path) -> list[str]:
    ...
    parts = sorted(root.glob("core_06-06_rights_part_*.md"))
    all_err: list[str] = []
    for path in parts:
...
def main() -> int:
    ...
    parts = sorted(root.glob("core_06-06_rights_part_*.md"))
    if not parts:
        print("subarticle-gloss audit: no core_06-06_rights_part_*.md under --root")
        return 1
