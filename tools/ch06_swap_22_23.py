...
def main() -> None:
    ...
    replacements = [
        ...
        (
            "*In plain terms: Section 2 creates **standing records** — focused case files about one sentient, institution, or situation over a clear time period. Each file is **axis-pure**: a **contribution standing record** for verified good, or a **violation standing record** for verified adverse findings — not both in one file. Related files may **cross-reference** each other; **section 2.3.2** lists what each file must contain and what counts as verified input. The files are not permanent popularity scores or vague labels. **Sections 2.3.4–2.3.6** cover versioning and implementation visibility; **section 2.3.7** states the forum boundary; **section 2.2** states the no-offset rule when linked contribution and violation records coexist.*",
            "*In plain terms: Section 2 creates **standing records** — focused case files about one sentient, institution, or situation over a clear time period. Each file is **axis-pure**: a **contribution standing record** for verified good, or a **violation standing record** for verified adverse findings — not both in one file. **Section 2.2** states the no-offset rule when linked contribution and violation records coexist. Related files must **cross-reference** each other under **section 2.3.1**; **section 2.3.2** lists what each file must contain and what counts as verified input. The files are not permanent popularity scores or vague labels. **Sections 2.3.4–2.3.6** cover versioning and implementation visibility; **section 2.3.7** states the forum boundary.*",
        ),
    ...
    for path in ROOT.rglob("*"):
        ...
        updated = updated.replace("Chapter Nine Chapter One §8.2 Standing Record operational requirements", "Chapter Nine Chapter One §8.3 Standing record operational requirements")
        ...
        updated = updated.replace("operational requirements in Chapter Nine Chapter One §8.2", "operational requirements in Chapter Nine Chapter One §8.3")
        updated = updated.replace("Chapter Nine section 2.2", "Chapter Nine section 2.3")
        if updated != original:
